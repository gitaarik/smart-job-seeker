...
export const GET: RequestHandler = async ({ url, setHeaders }) => {
  ...
  try {
    ...
    let filePath: string
    if (resumeToken.resumeType === 'resume') {
      filePath = path.join(process.cwd(), 'lib', 'resumes', 'resume', 'Resume Rik Wanders.pdf')
    } else {
      filePath = path.join(process.cwd(), 'lib', 'resumes', resumeToken.resumeType, 'Resume Rik Wanders.pdf')
    }

    // Check if file exists
    if (!fs.existsSync(filePath)) {
      throw error(404, 'Resume file not found. Please contact the administrator.')
    }
