<script lang="ts">
	...
	import { faCheck, faCircleNotch, faPen, faPlus } from '@fortawesome/free-solid-svg-icons';
	 ...
	 * way to put it on this job's document, or to move it once it is there.
	 *
	 ...
	 *
	 * The group and the position are asked, not assumed, but they open on the
	 * answer: right after the skill the match credited it through, where there is
	 * one. Appending was the first version, and it put "Monitoring" at the far
	 * end of a group that holds Sentry.
	 *
	 * With `word` it is the same form for a word already added: Save moves it,
	 * and "Take it off" removes it. The host page must expose `addSkillWord` and
	 * `removeSkillWord`.
	 */
	let {
		skill,
		...
		from?: string | null;
		/** The skill groups it could print in on this document, with the skills each prints. */
		groups: { id: number; name: string; skills: { id: number; name: string }[] }[];
		/** Where it would go by default. Ignored unless it is one of `groups`. */
		suggested?: number | null;
		/** The skill it relates to, which it opens placed right after. */
		anchor?: number | null;
		/** The word as this job's version already holds it; absent when adding. */
		word?: { id: number; categoryId: number; beforeSkillId: number | null } | null;
		docType: DocType;
	...
	/** The skill it goes in front of, as an option value; '' is the end. */
	let beforeId = $state('');

	let skillsIn = (id: string) => groups.find((g) => String(g.id) === id)?.skills ?? [];

	/**
	 * Every place in the group, each named by what it follows. Stored as the
	 * skill it precedes, so the start of the group is "before the first" and the
	 * end is no skill at all; "after the last" is the end, so it is not listed
	 * twice.
	 */
	let positions = $derived.by(() => {
		const skills = skillsIn(categoryId);
		if (skills.length === 0) return [{ value: '', label: 'At the end' }];
		return [
			{ value: String(skills[0].id), label: 'At the start' },
			...skills
				.slice(0, -1)
				.map((s, i) => ({ value: String(skills[i + 1].id), label: `After ${s.name}` })),
			{ value: '', label: 'At the end' }
		];
	});

	/** Right after the skill it relates to, when that skill is in this group. */
	function placeIn(id: string): string {
		if (word && String(word.categoryId) === id && word.beforeSkillId !== null) {
			const held = String(word.beforeSkillId);
			if (skillsIn(id).some((s) => String(s.id) === held)) return held;
		}
		if (word && String(word.categoryId) === id) return '';
		const skills = skillsIn(id);
		const at = skills.findIndex((s) => s.id === anchor);
		return at === -1 ? '' : String(skills[at + 1]?.id ?? '');
	}
	...
	function toggle() {
		...
		if (!open) return;
		error = null;
		const home = groups.find((g) => g.skills.some((s) => s.id === anchor))?.id;
		const start = word?.categoryId ?? home ?? suggested;
		categoryId = String(groups.find((g) => g.id === start)?.id ?? groups[0]?.id ?? '');
		beforeId = placeIn(categoryId);
	}
...
<span class="relative inline-block">
	{#if word}
		<!-- On the document already: its own colour, and a pen rather than a plus. -->
		<button
			type="button"
			onclick={toggle}
			title="On this job's {docLabel}. Click to move it or take it off"
			class="inline-flex cursor-pointer items-center gap-1.5 rounded-lg border border-[var(--dash-success)]/40 bg-[var(--dash-success-light)] px-2 py-0.5 align-middle text-xs text-[var(--dash-success)] transition-colors hover:border-[var(--dash-success)]"
		>
			<FontAwesomeIcon icon={faCheck} class="h-2.5 w-2.5" />
			{skill}
			<FontAwesomeIcon icon={faPen} class="h-2 w-2 opacity-60" />
		</button>
	{:else}
		<button
			type="button"
	...
	{#if open}
			...
			use:enhance={() => {
				...
				return async ({ result, update }) => {
					...
					if (result.type === 'failure') {
						error =
							(result.data as { error?: string } | undefined)?.error ?? "Couldn't save the word.";
						return;
					}
					open = false;
		...
		>
			...
			<p class="text-sm font-medium text-[var(--dash-text)]">
				{#if word}“{skill}” on this job's {docLabel}{:else}Add “{skill}” to this job's {docLabel}{/if}
			</p>
			...
			{#if groups.length > 0}
				<div>
					...
					<select
						...
						onchange={() => (beforeId = placeIn(categoryId))}
				...
				<div>
					<label
						for="word-position-{uid}"
						class="mb-1 block text-[10px] tracking-wide text-[var(--dash-text-muted)] uppercase"
						>Position</label
					>
					<select
						id="word-position-{uid}"
						name="before_skill_id"
						bind:value={beforeId}
						class="w-full cursor-pointer rounded border border-[var(--dash-border)] bg-transparent px-2 py-1.5 text-sm text-[var(--dash-text)] focus:ring-1 focus:ring-[var(--dash-primary)] focus:outline-none"
					>
			...
			<div class="flex items-center justify-end gap-1.5 pt-1">
				{#if word}
					<!-- The same form, sent to the other action: it carries word_id. -->
					<button
						type="submit"
						formaction="?/removeSkillWord"
						disabled={saving}
						class="mr-auto cursor-pointer text-xs text-[var(--dash-text-secondary)] transition-colors hover:text-[var(--dash-error)] hover:underline disabled:opacity-70"
					>
						Take it off
					</button>
				{/if}
