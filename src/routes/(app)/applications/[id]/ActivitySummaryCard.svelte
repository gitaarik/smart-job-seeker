<script lang="ts">
  ...
  let { summary, updatedAt, entryCount, activityHref }: {
    summary: string | null;
...
{#if summary || expected}
  <Card padding="lg">
    <div class="space-y-3">
      ...
      {#if summary}
        <p
          class="text-sm text-[var(--dash-text)] whitespace-pre-wrap leading-relaxed"
        >
          {summary}
        </p>

        <p
          class="flex flex-wrap items-center gap-1.5 text-xs text-[var(--dash-text-muted)]"
        >
          ...
          <span>
            ...
            >{entryCount} activity {entryCount === 1
                ? "entry"
                : "entries"}</a>{#if updatedAt}<span
      ...
      {:else}
        <p class="text-sm text-[var(--dash-text-muted)]">
          ...
          <a
            href={activityHref}
            class="underline hover:text-[var(--dash-primary)] transition-colors"
          >activity entry</a> changes, so adding or editing one will produce it.
        </p>
      {/if}
    </div>
  </Card>
