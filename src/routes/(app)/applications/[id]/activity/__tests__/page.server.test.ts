...
describe.each([
	...
	it('rejects a non-numeric application id', async () => {
		const res = await getAction()(createEvent(fields, { params: { id: 'abc' } }));
		expect(res).toMatchObject({ status: 400 });
	});
...
describe('create action — derivation', () => {
	...
	it('accepts an explicit type but never demands one', async () => {
		...
		expect(mockValues).toHaveBeenCalledWith(expect.objectContaining({ record_type: 'offer' }));
	});
	...
	it('ignores an unknown type rather than storing it', async () => {
		...
		expect(mockValues).toHaveBeenCalledWith(expect.objectContaining({ record_type: 'note' }));
	});
	...
	it('defers derivation for an upload until there is text', async () => {
		await actions.create!(createEvent({}, { file: new File(['x'], 'brief.pdf') }));
...
describe('update action', () => {
	...
	it('clears a field the form carried as empty', async () => {
		await actions.update!(createEvent({ id: '7', title: 'New title', step: '' }));
		const [[values]] = mockUpdateSet.mock.calls;
