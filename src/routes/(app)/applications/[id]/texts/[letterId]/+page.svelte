<script lang="ts">
	...
	async function postAction(letterId: number, action: string, fd: FormData) {
		const res = await fetch(`/applications/${appId}/texts/${letterId}?/${action}`, {
			method: 'POST',
	...
	async function apiSaveContent(letterId: number, content: string, deleteAfterVersionId?: number) {
	...
	<div class="flex flex-wrap items-center justify-between gap-3">
		...
		{#if !isNew}
			<button
				...
				class="flex items-center gap-1.5 rounded-lg border border-red-500/30 px-3 py-1.5 text-xs text-red-500 transition-colors hover:border-red-500/50 hover:bg-red-500/10"
			>
