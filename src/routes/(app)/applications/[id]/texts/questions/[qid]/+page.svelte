<script lang="ts">
	...
	async function postAction(action: string, fd: FormData) {
		const res = await fetch(`/applications/${appId}/texts/questions/${questionId}?/${action}`, {
			method: 'POST',
	...
	async function onDelete(versionId: number, scope: DeleteScope) {
		...
		fd.set('scope', scope);
		await postAction('deleteEntry', fd);
		await invalidateAll();
	}
...
<div class="mx-auto max-w-3xl space-y-6">
	...
	<Card padding="md">
		<form method="POST" action="?/saveQuestionText" use:enhance={handleQuestionSave}>
			<label for="q-text" class="mb-1 block text-sm font-medium text-[var(--dash-text-secondary)]"
				>Question</label
			>
			<div class="flex items-start gap-2">
				...
				{#if questionDirty}
					...
					>
						<FontAwesomeIcon icon={faCheck} class="h-3.5 w-3.5" /> Save question
					</button>
				{/if}
			</div>
		</form>
