...
export const load: PageServerLoad = async ({ parent, params }) => {
	...
	const question = application.application_questions?.find((q: { id: number }) => q.id === qid);
...
async function loadOwnedQuestion(
	...
	const question = await db.query.application_questions.findFirst({
		where: and(eq(application_questions.id, qid), eq(application_questions.application_id, appId))
	});
	if (!question) {
