...
let titleDirty = $derived(
  title.trim().length > 0 && title.trim() !== (data.sheet.title ?? ""),
);
...
async function postAction(action: string, fd: FormData) {
  const res = await fetch(
    `/applications/interview/cheatsheets/${cheatSheetId}?/${action}`,
...
<div class="max-w-3xl mx-auto space-y-6">
  ...
  <div class="flex items-center gap-3">
    <FontAwesomeIcon icon={faStickyNote} class="w-6 h-6 text-[var(--dash-primary)]" />
  ...
  <Card padding="md">
    <form method="POST" action="?/saveTitle" use:enhance={handleTitleSave}>
      <label for="sheet-title" class="block text-sm font-medium text-[var(--dash-text-secondary)] mb-1">
      ...
      <div class="flex items-start gap-2">
        ...
        {#if titleDirty}
          ...
          >
            <FontAwesomeIcon icon={faCheck} class="w-3.5 h-3.5" /> Save title
          </button>
        {/if}
      </div>
    </form>
