...
<div class="space-y-6">
  ...
  <!-- Demo template: the profile cloned into every demo user. Required before
       any link will work. -->
  <div
    class="rounded-lg border border-[var(--dash-border)] bg-[var(--dash-card)] p-4 space-y-3"
  >
    ...
    <p class="text-sm text-[var(--dash-text-secondary)]">
      The profile cloned into every demo user. {#if data.template.exists}
        Currently: <span class="font-medium text-[var(--dash-text)]">{
          data.template.profileName ?? "(no profile)"
        }</span>.
      {:else}
        <span class="text-amber-600">Not set yet — demo links won't work until
          you set one.</span>
      {/if}
    </p>
    {#if data.myProfiles.length > 0}
      ...
      >
        ...
        <button
          type="submit"
          class="px-3 py-2 text-sm rounded border border-[var(--dash-border)] text-[var(--dash-text)] hover:bg-[var(--dash-bg)]"
        >
