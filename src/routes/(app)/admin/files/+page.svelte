...
<div class="space-y-6">
	...
	{#if files.length === 0}
		<Card padding="lg">
			<p class="py-8 text-center text-sm text-[var(--dash-text-muted)]">No files found.</p>
		</Card>
	{:else}
		<div class="space-y-2">
				...
				<Card padding="sm">
					...
					>
						<div class="flex items-center gap-3">
							<!-- Type badge -->
							<span
								class="rounded-full px-2 py-0.5 text-xs {getTypeColor(file.type)} flex-shrink-0"
							>
								{getShortType(file.type)}
							</span>

							<!-- Filename -->
							<span class="min-w-0 flex-1 truncate text-sm text-[var(--dash-text)]">
							...
							<div class="flex flex-shrink-0 items-center gap-1.5">
								...
								{#if usageLabels.length === 0}
									<span
										class="rounded border border-amber-200 bg-amber-50 px-1.5 py-0.5 text-xs text-amber-600"
									>
										Orphaned
									</span>
								{/if}
							...
							<!-- Size & date -->
							<span class="w-16 flex-shrink-0 text-right text-xs text-[var(--dash-text-muted)]">
								{formatFileSize(file.filesize)}
							</span>
							<span
								class="hidden w-28 flex-shrink-0 text-right text-xs text-[var(--dash-text-muted)] sm:block"
							>
						...
						<div class="mt-3 space-y-3 border-t border-[var(--dash-border)] pt-3">
							...
							<div class="grid grid-cols-2 gap-3 text-xs sm:grid-cols-4">
								<div>
									...
									<p class="font-mono text-[10px] break-all text-[var(--dash-text)]">{file.id}</p>
								</div>
								<div>
									<span class="text-[var(--dash-text-muted)]">Disk name</span>
									<p class="break-all text-[var(--dash-text)]">{file.filename_disk || '—'}</p>
								</div>
								<div>
										...
										{#each file.importLogs as log}
											<div
												class="rounded p-2 text-xs {log.event.includes('error')
													...
													: 'border border-[var(--dash-border)] bg-[var(--dash-bg)]'}"
											>
												<div class="flex flex-wrap items-center gap-2">
													<span
														class="font-medium {log.event.includes('error')
													...
													<span class="text-[var(--dash-text-muted)]"
														>{formatDate(log.date_created)}</span
													>
												</div>
												{#if log.error}
												...
												{#if log.sections}
													<p class="mt-1 text-[var(--dash-text-muted)]">
														Sections: {Object.entries(log.sections as Record<string, number>)
															.map(([k, v]) => `${k}: ${v}`)
															.join(', ')}
													</p>
												{/if}
