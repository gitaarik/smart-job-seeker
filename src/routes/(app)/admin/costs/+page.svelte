...
<div class="space-y-6">
		...
		{#each data.availableMonths as month}
			<a
				...
				class="rounded-md px-3 py-1 text-sm transition-colors {data.currentPeriod === month.value
					...
					: 'border border-[var(--dash-border)] bg-[var(--dash-card)] text-[var(--dash-text-muted)] hover:border-[var(--dash-accent)]'}"
			>
				{month.label}
			</a>
		{/each}
	...
	<div class="grid grid-cols-2 gap-4 md:grid-cols-4">
		<Card padding="md">
			...
			<div class="text-2xl font-semibold text-[var(--dash-text)]">
				{formatUsd(data.summary.totalRevenueUsd)}
			</div>
		...
		<Card padding="md">
			...
			<div class="text-xs text-[var(--dash-text-muted)]">
				{formatNumber(data.summary.totalTransactions)} API calls
			</div>
		</Card>
		<Card padding="md">
			<div class="text-sm text-[var(--dash-text-muted)]">Margin</div>
			<div
				class="text-2xl font-semibold {data.summary.totalRevenueUsd - data.summary.totalCostUsd >= 0
					? 'text-green-500'
					: 'text-red-500'}"
			>
	...
	<Card padding="responsive">
		<h3 class="mb-4 text-lg font-semibold text-[var(--dash-text)]">Cost by Plan</h3>
				...
				{#each data.planStats as plan}
					...
					>
						<div>
							...
							<span class="ml-2 text-sm text-[var(--dash-text-muted)]">
						...
						<div class="text-right">
							<div class="flex items-center gap-4">
								...
								<div
									class="text-sm font-medium {plan.revenueUsd - plan.totalCostUsd >= 0
										? 'text-green-500'
										: 'text-red-500'}"
								>
	...
	<Card padding="responsive">
		<h3 class="mb-4 text-lg font-semibold text-[var(--dash-text)]">Cost by Provider</h3>
		{#if data.providerStats.length === 0}
			<p class="text-sm text-[var(--dash-text-muted)]">
				No provider data available. Run the backfill script or wait for new transactions.
			</p>
		{:else}
			<div class="space-y-3">
				{#each data.providerStats as ps}
					...
					>
						<div>
							...
							<span class="ml-1 text-sm text-[var(--dash-text-muted)]">/ {ps.model}</span>
						</div>
						<div class="text-right">
	...
	<!-- Top users by cost -->
	<Card padding="responsive">
		<h3 class="mb-4 text-lg font-semibold text-[var(--dash-text)]">Top Users by Cost</h3>
				...
				{#each data.topUsers as user, i}
					...
					>
						<div class="flex items-center gap-3">
							<span class="w-6 text-right text-sm text-[var(--dash-text-muted)]">{i + 1}.</span>
							<div>
								<span class="font-medium text-[var(--dash-text)]">{user.name || user.email}</span>
								<span class="ml-2 text-xs text-[var(--dash-text-muted)] capitalize"
									>{user.plan}</span
								>
							</div>
						</div>
