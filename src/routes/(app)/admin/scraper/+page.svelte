	...
	{:else}
				...
				<div class="space-y-2">
					...
					{#each healthChecks as issue}
						<div class="flex items-center justify-between gap-4">
							...
							>
								<span
									class="mt-1.5 h-2 w-2 flex-shrink-0 rounded-full {issue.severity === 'error'
										? 'bg-[var(--dash-error)]'
										: 'bg-amber-500'}"
								></span>
								<div>
									...
									{issue.label}
									{#if issue.details}
										<p class="mt-0.5 text-xs text-[var(--dash-text-muted)]">{issue.details}</p>
									{/if}
								</div>
							</div>
							{#if issue.fixAction}
								<button
									...
									class="flex-shrink-0 rounded-lg bg-[var(--dash-primary)]/10 px-2.5 py-1 text-xs text-[var(--dash-primary)] transition-colors hover:bg-[var(--dash-primary)]/20 disabled:opacity-50"
								>
		...
		<div class="flex gap-2">
			<button
				...
				class="rounded-lg px-3 py-1.5 text-xs transition-colors {statusFilter === ''
			...
			<button
				...
				class="rounded-lg px-3 py-1.5 text-xs transition-colors {statusFilter === 'active'
					? 'bg-[var(--dash-primary)] text-white'
					: 'bg-[var(--dash-bg)] text-[var(--dash-text-secondary)] hover:bg-[var(--dash-border)]'}"
			>
				Active
			...
			<button
				...
				class="rounded-lg px-3 py-1.5 text-xs transition-colors {statusFilter === 'failed'
					? 'bg-[var(--dash-primary)] text-white'
					: 'bg-[var(--dash-bg)] text-[var(--dash-text-secondary)] hover:bg-[var(--dash-border)]'}"
			>
				Failed / Cancelled
		...
		{#if runs.length === 0}
			<Card padding="responsive">
				<p class="py-4 text-center text-sm text-[var(--dash-text-muted)]">No runs found.</p>
			</Card>
		{:else}
			<div class="space-y-2">
						...
						<div class="flex items-start justify-between gap-4">
							...
							<div class="min-w-0 flex-1">
								...
								<div class="mb-1 flex items-center gap-2">
									...
									{:else}
										<span class="h-2 w-2 flex-shrink-0 rounded-full {statusDot(run.status)}"></span>
									{/if}
									<span class="text-xs font-medium {statusColor(run.status)} uppercase"
										>{run.status}</span
									>
									<a
								...
								<!-- Details row -->
								<div
									class="flex flex-wrap gap-x-4 gap-y-1 text-xs text-[var(--dash-text-secondary)]"
								>
									...
									<span>
										{#if run.searchTask.browserProvider === 'hosted'}
											<FontAwesomeIcon icon={faCloud} class="h-3 w-3" /> Cloud
										{:else}
											<FontAwesomeIcon icon={faDesktop} class="h-3 w-3" /> Desktop
										{/if}
									</span>
								...
								<!-- Time + duration -->
								<div
									class="mt-1 flex flex-wrap gap-x-4 gap-y-1 text-xs text-[var(--dash-text-muted)]"
								>
									<span>{formatTime(run.startedAt)}</span>
									<span
										>{formatDuration(run.startedAt, run.finishedAt)}{isActive(run.status)
											? ' (running)'
											: ''}</span
									>
								...
								{#if run.errorMessage && run.status !== 'cancelled'}
									<p
										class="mt-1 truncate text-xs text-[var(--dash-error)]"
							...
							<div class="flex flex-shrink-0 items-center gap-2">
								{#if run.liveUrl}
									<a
										...
										target="_blank"
										rel="noopener"
										class="rounded p-1.5 text-[var(--dash-text-muted)] transition-colors hover:bg-[var(--dash-bg)] hover:text-[var(--dash-primary)]"
								...
								{#if isActive(run.status)}
									<button
										...
										class="flex items-center gap-1.5 rounded-lg bg-[var(--dash-error)]/10 px-2.5 py-1.5 text-xs text-[var(--dash-error)] transition-colors hover:bg-[var(--dash-error)]/20 disabled:opacity-50"
										title={run.status === 'stopping'
											? 'Stop signal sent, waiting for scraper to finish'
											: 'Stop this run'}
									>
								...
								{:else}
									<button
										...
										class="flex items-center gap-1.5 rounded-lg bg-[var(--dash-primary)]/10 px-2.5 py-1.5 text-xs text-[var(--dash-primary)] transition-colors hover:bg-[var(--dash-primary)]/20 disabled:opacity-50"
