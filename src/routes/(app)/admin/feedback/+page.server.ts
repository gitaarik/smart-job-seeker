...
export const load: PageServerLoad = async ({ url }) => {
	...
	const [
		...
		db
			.select({ reviewedCount: count() })
			.from(user_feedback)
			.where(eq(user_feedback.status, 'reviewed')),
		db
			.select({ waitingCount: count() })
			.from(user_feedback)
			.where(eq(user_feedback.status, 'waiting')),
		db
			.select({ resolvedCount: count() })
			.from(user_feedback)
			.where(eq(user_feedback.status, 'resolved'))
	]);
...
export const actions: Actions = {
	...
	addNote: async ({ request }) => {
		...
		await db
			.update(user_feedback)
			.set({ admin_note: note, date_updated: new Date() })
			.where(eq(user_feedback.id, id));
		return { success: true };
	},
	...
	reply: async ({ request, locals }) => {
		...
		if (feedbackEntry) {
			const recipientIds = new Set([
				...
				...feedbackEntry.user_feedback_subscribers.map((s) => s.user_id)
			]);
	...
	merge: async ({ request }) => {
		...
		await db
			.insert(user_feedback_subscribers)
		...
		await db
			...
			.where(eq(user_feedback.id, targetId));
