	...
	upload: async ({ request, locals }) => {
		...
		if (file.type === 'application/json' || file.name.endsWith('.json')) {
			try {
				...
				const message = error instanceof Error ? error.message : 'Failed to parse JSON Resume file';
		...
		if (!isSupportedMimeType(file.type)) {
			return fail(400, {
				error: `Unsupported file type: ${file.type}. Please upload a PDF, DOCX, or HTML file.`
			});
		}
