<script lang="ts">
	...
	function getReplacedPublicResumeName(): string | null {
		if (!data.publicResumeVersionId || data.publicResumeVersionId === version.id) return null;
		const v = data.allVersions.find((v) => v.id === data.publicResumeVersionId);
		return v?.name || v?.slug || null;
	}

	function getReplacedPublicCvName(): string | null {
		if (!data.publicCvVersionId || data.publicCvVersionId === version.id) return null;
...
<div class="space-y-6">
	...
	<div class="flex items-center gap-2">
		<FontAwesomeIcon icon={faFileAlt} class="h-5 w-5 text-indigo-600" />
	...
	{#if form?.success}
		<div
			class="rounded-lg border border-[var(--dash-success)] bg-[var(--dash-success-light,#dcfce7)] p-4"
		>
			<p class="text-sm text-[var(--dash-success)]">Version updated.</p>
		</div>
	{/if}
	...
	<form method="POST" action="?/update" use:enhance>
		<Card padding="responsive">
			<div class="space-y-6">
				<!-- Basic Info -->
				<div>
					<h3 class="mb-4 text-sm font-semibold tracking-wide text-[var(--dash-text)] uppercase">
						Basic Info
					</h3>
					<div>
					...
					<button
						...
						class="mt-3 text-xs text-[var(--dash-text-secondary)] transition-colors hover:text-[var(--dash-primary)]"
					>
						{showAdvanced ? 'Hide' : 'Show'} advanced options
					</button>

					{#if showAdvanced}
						<div class="mt-3 space-y-4 border-t border-[var(--dash-border)] pt-4">
							<div>
								<label
									for="edit-slug"
				...
				<!-- Public Access -->
				<div>
					<h3 class="mb-1 text-sm font-semibold tracking-wide text-[var(--dash-text)] uppercase">
						Public Access
					</h3>
					...
					<div class="space-y-2">
						<label class="flex cursor-pointer items-start gap-1.5 text-sm text-[var(--dash-text)]">
							<input
								type="checkbox"
								name="publicResume"
								bind:checked={editPublicResume}
								class="mt-0.5 rounded border-[var(--dash-border)] text-[var(--dash-primary)] focus:ring-[var(--dash-primary)]"
							/>
							<span>
				...
				<div>
					...
					<p class="mb-3 text-xs text-[var(--dash-text-secondary)]">
						Uncheck a field to hide it on this version's resume &amp; CV. Fields you leave empty on
						...
						{#each CONTACT_FIELDS as field}
							...
							>
								<input
									...
									onchange={(e) => (editContactVisible[field.key] = e.currentTarget.checked)}
									class="rounded border-[var(--dash-border)] text-[var(--dash-primary)] focus:ring-[var(--dash-primary)]"
								/>
								{field.label}
							</label>
	...
	<Card padding="responsive">
		...
		{#if !hasTaggedItems}
			<p class="text-sm text-[var(--dash-text-secondary)]">
				No items are tagged with this version yet. Add the <code
					class="rounded bg-[var(--dash-bg)] px-1.5 py-0.5 font-mono text-xs">{version.slug}</code
				> tag to work experiences, education, or side projects to include them in this version.
			</p>
		{:else}
				...
				{#if tagUsage.workExperiences.length > 0}
					<div>
						<div class="mb-2 flex items-center gap-1.5">
							...
							/>
							<span
								class="text-xs font-medium tracking-wide text-[var(--dash-text-secondary)] uppercase"
