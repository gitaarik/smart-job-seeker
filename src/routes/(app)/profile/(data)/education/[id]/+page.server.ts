...
export const load: PageServerLoad = async ({ params, parent }) => {
	...
	if (isNaN(id)) {
		redirect(302, '/profile/education');
	}
	...
	// Get logo URL
	const logoUrl = entry?.logo_path ? `/uploads/${entry.logo_path}` : null;

	// Get banner URL
	const bannerUrl = entry?.banner_path ? `/uploads/${entry.banner_path}` : null;
