...
<svelte:head>
	<title>Side Projects - Profile - Smart Job Seeker</title>
</svelte:head>
		...
		>
			...
			<div class="space-y-4">
				<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
					...
					<div>
						<label for="new-stars" class="mb-1 block text-sm font-medium text-[var(--dash-text)]">
							GitHub Stars
						</label>
						<input
							type="number"
	...
	{:else}
		...
		>
			{#snippet row(project)}
				...
				{#if project.stars}
					<span class="flex-shrink-0 text-sm text-amber-500">
				...
				{#each projects as project (project.id)}
					...
					>
						{#snippet title()}
							{project.name}
							{#if project.stars}
								<span class="ml-2 text-sm text-amber-500">
									<FontAwesomeIcon icon={faStar} class="h-3 w-3" />
									{project.stars}
								</span>
							{/if}
						...
						{#snippet subtitle()}
							<span class="max-w-[200px] truncate sm:max-w-none">
						...
						{#snippet dateline()}
							{formatDisplayDate(project.start_date) || 'N/A'} – {formatDisplayDate(
								project.end_date
							) || 'Present'}
						{/snippet}
