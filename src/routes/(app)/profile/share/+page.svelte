<script lang="ts">
	...
	function isEditDirty(): boolean {
		...
		return (
			...
			editVersion !== originalValues.version ||
			editFormat !== originalValues.format ||
			editViewMode !== originalValues.viewMode ||
			editVisitLimit !== originalValues.visitLimit ||
			editExpiresAt !== originalValues.expiresAt ||
	...
	function startEdit(token: (typeof tokens)[0]) {
		...
		editExpiresAt = token.expires_at ? new Date(token.expires_at).toISOString().split('T')[0] : '';
...
<div class="space-y-6">
	...
	{:else}
		...
		{#if showAddForm}
			...
			>
				<h3 class="mb-4 font-medium text-[var(--dash-text)]">Create Private Link</h3>
				<div class="space-y-4">
					...
					<div>
						<label for="new-notes" class="mb-1 block text-sm font-medium text-[var(--dash-text)]">
							Notes
						</label>
						<textarea
							id="new-notes"
		...
		{:else if tokens.length > 0}
			<div class="space-y-3">
				{#each tokens as token (token.id)}
					...
					{@const limitReached = token.visit_limit && token.visit_count >= token.visit_limit}
					<ItemCard
						id={token.id}
						{expandedId}
						onToggle={toggleExpand}
						icon={faLink}
						iconColor={expired || limitReached
					...
					>
						...
						{#snippet badges()}
							{#if expired}
								<span
									class="rounded-full bg-[var(--dash-error-light)] px-2 py-0.5 text-xs text-[var(--dash-error)]"
								>
									Expired
								</span>
							...
							{:else}
								<span
									class="rounded-full bg-[var(--dash-success-light)] px-2 py-0.5 text-xs text-[var(--dash-success)]"
								>
									Active
								</span>
							{/if}
						...
						{#snippet headerActions()}
							...
							>
								<FontAwesomeIcon
									...
									class="h-4 w-4 {copiedId === token.id ? 'text-green-600' : ''}"
								/>
							</button>
							<button
								type="button"
							...
							{:else}
								...
								<div class="space-y-4">
									...
									<div>
										...
										<div class="flex items-center gap-2 rounded-lg bg-[var(--dash-bg)] p-2">
											<code class="flex-1 truncate text-sm text-[var(--dash-text)]">
												{getPrivateLinkUrl(token.token)}
											</code>
											<button
												type="button"
												onclick={() => copyToClipboard(token.token, token.id)}
												class="rounded bg-[var(--dash-primary)] px-3 py-1 text-sm text-white transition-colors hover:bg-[var(--dash-primary-hover)]"
											>
									...
									<div class="grid grid-cols-2 gap-4 text-sm md:grid-cols-4">
										...
										<div>
											<p class="text-[var(--dash-text-secondary)]">Last Accessed</p>
											<p class="font-medium text-[var(--dash-text)]">
