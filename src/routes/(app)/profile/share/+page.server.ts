...
export const actions: Actions = {
	create: async ({ request, locals, cookies }) => {
		...
		const version = await db.query.profile_versions.findFirst({
			where: and(
				eq(profile_versions.id, parseInt(profile_version)),
	...
	update: async ({ request, locals, cookies }) => {
		...
		const version = await db.query.profile_versions.findFirst({
			where: and(
				eq(profile_versions.id, existingToken.profile_version),
				eq(profile_versions.profile_id, profileId)
			)
		});
		...
		await db
			...
			.set({
				...
				view_mode:
					view_mode && isValidViewMode(view_mode)
						? view_mode
						: existingToken.view_mode || DEFAULT_VIEW_MODE,
				visit_limit: visit_limit ? parseInt(visit_limit) : null,
				expires_at: expires_at ? new Date(expires_at) : null,
