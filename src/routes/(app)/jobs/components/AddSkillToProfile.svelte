<script lang="ts">
  ...
  function toggleVersion(slug: string) {
    versions = versions.includes(slug)
      ? versions.filter((v) => v !== slug)
      : [...versions, slug];
  }
    ...
    >
      ...
      {#if editing && strength === null}
        <p class="text-[10px] leading-snug text-[var(--dash-text-muted)]">
          This job's match doesn't count it yet — that catches up the next time
          the job is matched.
        </p>
      {/if}
      ...
      <div>
        ...
        <p class="mt-0.5 text-[10px] leading-snug text-[var(--dash-text-muted)]">
          ...
          {:else}
            Counts for job matching. Stays off your resume / CV until you turn
            this on.
          {/if}
        </p>
        ...
        {#if !showOnCv && versionSlugs.length > 0}
          ...
          <p
            class="mt-2 mb-1 text-[10px] tracking-wide text-[var(--dash-text-muted)] uppercase"
          >
            Show anyway on
          </p>
          <div class="flex flex-wrap gap-1.5">
            {#each versionSlugs as slug}
              <button
                ...
                class="
                  cursor-pointer rounded border px-2 py-1 text-xs transition-colors {versions
                  .includes(slug)
