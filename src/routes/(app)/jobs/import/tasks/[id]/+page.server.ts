...
import { platform_credentials, profiles, search_tasks } from '$lib/server/db/schema';
...
export const load: PageServerLoad = async ({ params, parent }) => {
	...
	const searchTask = {
		...searchTaskRow,
		platform_credential_id: searchTaskRow.platform_profile?.platform_credential_id ?? null
	};
	...
	if (user && searchTask.platform_id) {
		...
		for (const s of sharedCreds) {
			...
			const ownerLabel =
				s.platform_credential.owner?.name || s.platform_credential.owner?.email || 'a contact';
	...
	const effectiveCountryCode =
		profileData?.browser_country_code || profileData?.country_code || 'US';
	...
	const profileSkillLevels = await getProfileSkillLevels(layoutData.selectedProfile.id);
