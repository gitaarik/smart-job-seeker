...
describe('updateDetails action', () => {
	...
	it('rejects unauthenticated', async () => {
		const res = await actions.updateDetails!(createEvent({ fields: detailsForm(), user: null }));
	...
	it('rejects a scraped job', async () => {
		...
		const res = await actions.updateDetails!(createEvent({ fields: detailsForm() }));
	...
	it('canonicalizes a salary period alias', async () => {
		await actions.updateDetails!(createEvent({ fields: detailsForm({ salary_period: 'yearly' }) }));
		expect(mockUpdateSet).toHaveBeenCalledWith(expect.objectContaining({ salary_period: 'year' }));
	});
	...
	it('rejects a malformed date rather than writing it', async () => {
		...
		expect(mockUpdateSet).toHaveBeenCalledWith(expect.objectContaining({ date_posted: null }));
	});
	...
	it('re-resolves the platform from a changed URL', async () => {
		...
		expect(mockDetectPlatformId).toHaveBeenCalledWith('https://www.linkedin.com/jobs/1');
		expect(mockUpdateSet).toHaveBeenCalledWith(expect.objectContaining({ job_platform_id: 7 }));
	});
	...
	it('keeps a project duration only while the period is still a project', async () => {
		...
		await actions.updateDetails!(createEvent({ fields: detailsForm({ salary_period: 'hour' }) }));
		expect(mockUpdateSet).toHaveBeenCalledWith(
...
describe('updateDescription action', () => {
	...
	it('rejects unauthenticated', async () => {
		const res = await actions.updateDescription!(createEvent({ description: 'text', user: null }));
		expect(res).toMatchObject({ status: 401 });
	});
	...
	it('rejects a scraped job', async () => {
		mockJobFindFirst.mockResolvedValueOnce({ created_manually: false });
		const res = await actions.updateDescription!(createEvent({ description: 'text' }));
		expect(res).toMatchObject({ status: 403 });
		expect(mockUpdate).not.toHaveBeenCalled();
	});
	...
	it('rejects a blank description', async () => {
		const res = await actions.updateDescription!(createEvent({ description: '   ' }));
		expect(res).toMatchObject({ status: 400 });
		expect(mockUpdate).not.toHaveBeenCalled();
	});
	...
	it('re-parses the newly saved text and re-scores when asked', async () => {
		...
		mockJobFindFirst.mockResolvedValueOnce({ created_manually: true }).mockResolvedValueOnce({
			source_html_stripped: 'New description',
			job_description: 'New description',
			source_url: null,
			title: 'Old Title',
