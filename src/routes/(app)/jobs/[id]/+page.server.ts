...
> {
	...
	const effectiveLocation = rawLocation && normalizeWorkLocation(rawLocation) ? null : rawLocation;
	...
	try {
		await addMatchJob({ profileId, jobId, triggeredBy: 'user' }, RESCORE_TIMEOUT_MS);
	} catch (err) {
		console.warn(
...
export const load: PageServerLoad = async ({ parent, params }) => {
	...
	const jobCategory =
		jobStatus?.status === 'saved' ? 'saved' : match && match.score > 0 ? 'matches' : 'all';
	...
	const [profileSkillLevels, profileSkillIndex] = await Promise.all([
	...
	const chatContext = {
		label: `Job: ${job.title ?? 'Untitled'}${job.company ? ` at ${job.company}` : ''}`
	};
