<script lang="ts">
			...
			children: [
				{
					...
					icon: faExchangeAlt,
					alsoActiveFor: ['/data/profile-export', '/data/settings-import', '/data/settings-export']
				},
				{
	...
	let isAdmin = $derived(($page.data.user as { is_admin?: boolean })?.is_admin ?? false);
...
<button
	...
	class="fixed right-4 bottom-4 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-[var(--dash-primary)] text-white shadow-lg transition-colors hover:bg-[var(--dash-primary-hover)] lg:hidden"
	aria-label={mobileMenuOpen ? 'Close menu' : 'Open menu'}
>
	<FontAwesomeIcon icon={mobileMenuOpen ? faTimes : faBars} class="h-6 w-6" />
</button>

<!-- Sidebar -->
<aside
	class="
    fixed left-0 z-40 w-56 transform border-r border-[var(--dash-border)] bg-[var(--dash-card)] transition-transform duration-200 ease-in-out
	...
	<nav class="h-full overflow-y-auto p-3 pb-16">
				...
				{#if item.children}
					...
					<li>
						<button
							...
							class="
                  flex w-full items-center justify-between rounded-lg px-2.5 py-1.5 text-left text-sm transition-colors {isChildActive(
								item
							)
								...
								{#each item.children as child}
									<li>
										<a
											...
											class="
                          flex items-center gap-2 rounded-lg px-2.5 py-1.5 transition-colors {isActive(
												child.href,
												child.alsoActiveFor
											)
				...
				{:else}
					...
					<li>
						<a
							...
							onclick={closeMobileMenu}
							class="
                  flex items-center gap-2 rounded-lg px-2.5 py-1.5 text-sm transition-colors {isActive(
								item.href,
								item.alsoActiveFor
							)
								? 'bg-[var(--dash-primary)] text-white'
        ...
        "
			>
				<FontAwesomeIcon
					icon={planIcon.icon}
					class="h-4 w-4 {$page.url.pathname.startsWith('/billing')
						? planIcon.activeColor
						: planIcon.color}"
				/>
			...
			{#if creditBalance}
				...
				{@const usedPct =
					total > 0 ? Math.min(100, Math.round((creditBalance.used / total) * 100)) : 0}
				{@const daysLeft = creditBalance.periodEnd
					? Math.max(
							0,
							Math.ceil((new Date(creditBalance.periodEnd).getTime() - Date.now()) / 86400000)
						)
					: null}
				<a
					href="/billing"
				...
				>
					<div
						class="mb-1 flex items-center justify-between text-[10px] text-[var(--dash-text-secondary)]"
					>
						<span>{usedPct}% used</span>
						{#if daysLeft != null}<span>resets in {daysLeft}d</span>{/if}
					</div>
					<div class="h-1 overflow-hidden rounded-full bg-[var(--dash-border)]">
						<div
							class="h-full rounded-full transition-all {usedPct >= 90
								? 'bg-red-500'
								: usedPct >= 75
									? 'bg-amber-500'
									: 'bg-[var(--dash-primary)]'}"
		...
		<div class="mt-4 border-t border-[var(--dash-border)] pt-3">
			<button
				...
				class="flex w-full items-center gap-2 rounded-lg px-2.5 py-1.5 text-sm text-[var(--dash-primary)] transition-colors hover:bg-[var(--dash-primary)]/10"
			>
		...
		{#if isAdmin}
			...
			<div class="mt-3 border-t border-amber-500/30 pt-3">
				<button
					type="button"
          ...
          "
				>
					<div class="flex items-center gap-2">
						<FontAwesomeIcon icon={adminItem.icon} class="h-4 w-4" />
		...
		<div class="mt-2 border-t border-[var(--dash-border)] pt-2 lg:hidden">
			<button
				type="button"
				onclick={() => {
					closeMobileMenu();
				...
				class="flex w-full items-center gap-2 rounded-lg px-2.5 py-1.5 text-left text-sm text-[var(--dash-text)] transition-colors hover:bg-[var(--dash-bg)]"
			>
