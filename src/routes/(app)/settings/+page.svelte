<script lang="ts">
	...
	const pwMismatch = $derived(confirmPassword.length > 0 && newPassword !== confirmPassword);
	...
	const resolvedFormat = $derived(resolveTimeFormat(timeFormat, timezone));
		...
		<div class="space-y-5">
			...
			{#if !emailOpen}
				<div class="flex items-center gap-3">
					<p class="w-20 shrink-0 text-sm font-medium text-[var(--dash-text)]">Email</p>
					<p class="text-sm text-[var(--dash-text-muted)]">{originalEmail}</p>
					<button
						type="button"
						onclick={() => (emailOpen = true)}
						class="text-[var(--dash-text-muted)] transition-colors hover:text-[var(--dash-text)]"
						title="Change email"
					>
				...
				<div class="space-y-4">
					<div>
						...
						<input
							...
							id="account-email"
							bind:value={email}
							class="w-full max-w-sm rounded-md border border-[var(--dash-border-input)] bg-[var(--dash-card)] px-3 py-2 text-sm text-[var(--dash-text)] focus:border-transparent focus:ring-2 focus:outline-none"
					...
					{#if emailSaved}
						<p class="text-sm" style="color: var(--dash-success);">
							Verification email sent to <strong>{email}</strong>. Please check your inbox to
							confirm the change.
						</p>
					{/if}
					...
					<div class="flex items-center gap-3">
						<button
							...
							class="flex items-center gap-2 rounded-lg bg-[var(--dash-primary)] px-4 py-2 text-sm font-medium text-white transition-colors hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50"
						>
							{#if emailSaving}
						...
						<button
							...
							class="px-4 py-2 text-sm font-medium text-[var(--dash-text-muted)] transition-colors hover:text-[var(--dash-text)]"
						>
							Cancel
						</button>
					</div>
			...
			{#if !pwOpen}
				<div class="flex items-center gap-3">
					<p class="w-20 shrink-0 text-sm font-medium text-[var(--dash-text)]">Password</p>
					<p class="text-sm tracking-wider text-[var(--dash-text-muted)]">••••••••••••</p>
					<button
						type="button"
						onclick={() => (pwOpen = true)}
						class="text-[var(--dash-text-muted)] transition-colors hover:text-[var(--dash-text)]"
			...
			{:else}
				<div class="space-y-4">
					<div>
						...
						<div class="relative w-full max-w-sm">
							<input
								...
								class="w-full rounded-md border border-[var(--dash-border-input)] bg-[var(--dash-card)] px-3 py-2 pr-10 text-sm text-[var(--dash-text)] focus:border-transparent focus:ring-2 focus:outline-none"
								style="--tw-ring-color: var(--dash-primary);"
							/>
							<button
								type="button"
								onclick={() => (showCurrentPassword = !showCurrentPassword)}
								class="absolute top-1/2 right-2.5 -translate-y-1/2 text-[var(--dash-text-muted)] hover:text-[var(--dash-text)]"
								tabindex={-1}
							>
								<FontAwesomeIcon icon={showCurrentPassword ? faEyeSlash : faEye} class="h-4 w-4" />
							</button>
						</div>
					</div>
					...
					<div>
						...
						<div class="relative w-full max-w-sm">
							...
							<button
								...
								tabindex={-1}
							>
								<FontAwesomeIcon icon={showNewPassword ? faEyeSlash : faEye} class="h-4 w-4" />
							</button>
						</div>
					...
					<div>
						...
						/>
						{#if pwMismatch}
							<p class="mt-1 text-xs" style="color: var(--dash-error);">Passwords don't match</p>
						{/if}
					</div>
				...
				<div>
					...
					>
						...
						<button
							...
							class="border-x border-[var(--dash-border-input)] px-3 py-1.5 text-sm transition-colors {timeFormat ===
							'12h'
								? 'bg-[var(--dash-primary)] text-white'
								: 'bg-[var(--dash-card)] text-[var(--dash-text)] hover:bg-[var(--dash-bg-hover)]'}"
						>
