...
describe('POST /api/profile-skills — category choice', () => {
	beforeEach(() => {
		...
		mockInsertReturning.mockResolvedValueOnce([{ id: 99 }]).mockResolvedValue([{ id: 500 }]);
	});
	...
	it('creates a category from a typed name, after the last one', async () => {
		const res = await POST(createEvent({ name: 'Rust', category_name: 'Data & ML' }));
	...
	it('reuses an existing category of that name', async () => {
		...
		const res = await POST(createEvent({ name: 'Rust', category_name: '  bAcKeNd  ' }));
