...
import { CHEATSHEET_VERSIONS, trimVersionsFrom } from '$lib/server/ai-chat/entity-versions';
...
export const POST: RequestHandler = async ({ params, request, locals }) => {
	...
	if (!sheet || profile?.user_id !== user.id) {
		return json(
			{ success: false, message: 'Cheat sheet not found' },
			{
				status: 404
			}
		);
	...
	const result = await trackGeneration('cheatsheet', cheatSheetId, mode ?? 'followup', () =>
		restartMode
