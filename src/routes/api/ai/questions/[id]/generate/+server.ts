...
import { parseBody, questionGenerateSchema } from '$lib/server/validation/api-schemas';
...
export const POST: RequestHandler = async ({ params, request, locals }) => {
	...
	const { commit, mode, instructions } = parseBody(questionGenerateSchema, body);

	await requireCredits(user.id, 5);

	const result = await trackGeneration('question', questionId, mode, () =>
