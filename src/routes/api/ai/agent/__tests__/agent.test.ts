			...
			insert: (table: { _name?: string }) => ({
				values: (rows: unknown) => {
					if (table?._name === 'agent_message_proposals') {
						...
						return {
							returning: () => Promise.resolve(list.map((_, i) => ({ id: 900 + i })))
						};
					}
...
function event(message = 'What does this job ask for?', overrides: Record<string, unknown> = {}) {
	return {
...
function callArgs() {
	const [, promptKey, customVariables, , options] = mockCreateAndGenerate.mock.calls[0];
...
describe('capability routing', () => {
	...
	it('switches template and schema when something is proposable', async () => {
		...
		expect(callArgs().customVariables).toHaveProperty('capabilities', 'CAPABILITY BLOCK');
		...
		expect(mockProposalInsert).toHaveBeenCalledWith([expect.objectContaining({ message_id: 101 })]);
	});
...
describe('degrading a bad structured reply', () => {
	...
	it('persists the notice with the message, so a reload still shows it', async () => {
		...
		const rows = mockInsertValues.mock.calls.map(([v]) => v).find((v) => Array.isArray(v)) as {
			role: string;
			content: string;
		}[];
