...
describe('requireConversationProfile', () => {
	it("returns the profile when it is the caller's", async () => {
		...
		await expect(requireConversationProfile(url('?profile_id=12'), 'user-1')).resolves.toBe(12);
	...
	it('rejects a missing profile_id rather than guessing one', async () => {
		await expect(requireConversationProfile(url(''), 'user-1')).rejects.toMatchObject({
			status: 400
		});
	});
	...
	it.each([
		...
		await expect(requireConversationProfile(url(qs), 'user-1')).rejects.toMatchObject({
			status: 400
		});
	});
	...
	it("propagates the 403 when the profile is not the caller's", async () => {
		...
		await expect(requireConversationProfile(url('?profile_id=99'), 'user-1')).rejects.toMatchObject(
			{ status: 403 }
		);
	});
});
