...
import {
  getAllMatcherStates,
  isMatcherAlive,
} from "$lib/server/job/matcher-state";
...
export const GET: RequestHandler = async ({ request }) => {
  ...
  const [
    ...
    db.select({
      ...
      .innerJoin(
        search_task_runs,
        eq(search_task_runs.id, search_task_run_items.run_id),
      )
      .where(and(
  ...
  const activeBullIds = new Set(
    allActive.map((j) => j.id).filter(Boolean) as string[],
  );
  ...
  const stuckStopping = activeRuns
    ...
    .map((r) => ({
      ...r,
      ageSeconds: r.started_at
        ? Math.floor((now - r.started_at.getTime()) / 1000)
        : null,
    }))
