...
export const GET: RequestHandler = async ({ locals, url }) => {
	...
	const userIds = [
		...new Set(runs.map((r) => r.search_task.profile.user_id).filter(Boolean) as string[])
	];
	const userList =
		userIds.length > 0
...
async function getHealthChecks(): Promise<HealthIssue[]> {
	...
	const [orphanedItems, stuckRunsResult, statusMismatches, queueOrphans] = await Promise.all([
		...
		const [run] = await db
			...
			.values({
				...
				settings: {
					...
					stop_after_duplicates: (searchTask as Record<string, unknown>).stop_after_duplicates as
						number | null,
