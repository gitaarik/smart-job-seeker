...
describe('POST /api/interview-stories', () => {
	...
	it('rejects missing profile_id', async () => {
		await expect(POST(createEvent('POST', { title: 'Test' }))).rejects.toMatchObject({
			status: 400
		});
	});
	...
	it('rejects empty title', async () => {
		await expect(POST(createEvent('POST', { profile_id: 1, title: '' }))).rejects.toMatchObject({
			status: 400
		});
	});
	...
	it('starts sort at 0 when no existing stories', async () => {
		...
		expect(mockInsertValues).toHaveBeenCalledWith(expect.objectContaining({ sort: 0 }));
	});
	...
	it("bumps the profile's date_updated so the match snapshot re-exports", async () => {
		...
		expect(mockUpdateFn).toHaveBeenCalledWith(expect.objectContaining({ id: 'profiles.id' }));
...
describe('PUT /api/interview-stories', () => {
	...
	it('rejects missing story ID', async () => {
		await expect(PUT(createEvent('PUT', { profile_id: 1, title: 'Test' }))).rejects.toMatchObject({
			status: 400
		});
	});
