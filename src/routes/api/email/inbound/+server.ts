...
import { createRateLimitResponse, webhookRateLimiter } from '$lib/server/middleware/rate-limit';
	...
	try {
		...
		if (contentType.includes('application/json')) {
			...
			if (peekBody.type === 'webhook_verification' && peekBody.verification_token) {
				console.log(
					'[email/inbound] Webhook verification request, token:',
					peekBody.verification_token
				);
		...
		if (contentType.includes('application/json')) {
			...
			if (!recipient) {
				...
				fromAddress = String(
					envelope.from || body.from || body.sender || headers.from || fromAddress || ''
				);
			...
			bodyText =
				(formData.get('body-plain') as string) || (formData.get('stripped-text') as string) || null;
			bodyHtml =
				(formData.get('body-html') as string) || (formData.get('stripped-html') as string) || null;
		} else {
		...
		console.log(
			`[email/inbound] Received email to=${recipient} from=${fromAddress} subject="${subject || ''}"`
		);
