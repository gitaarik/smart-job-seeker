...
export const POST: RequestHandler = async ({ locals, cookies, request }) => {
  ...
  try {
    ...
    console.warn(
      `[parse-description] extraction threw for profile ${profileId}:`,
      err,
    );
  }
  ...
  if (!parsed) {
    return json({
      ok: false,
      message:
        "We couldn't read that posting automatically — you can fill in the details yourself.",
    });
  }
  ...
  const officeLocation = rawLocation && normalizeWorkLocation(rawLocation)
    ? null
    : rawLocation;
