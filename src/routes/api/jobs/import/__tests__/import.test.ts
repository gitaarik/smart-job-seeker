 ...
 */
function createMockRequest(body: Record<string, unknown>, apiKey?: string): Request {
 ...
 */
function createMockEvent(request: Request, user?: App.Locals['user']) {
	return {
		request,
		locals: { user },
...
describe('POST /api/jobs/import - Single Job Import', () => {
	...
	describe('authentication', () => {
		it('should reject request without authentication', async () => {
			mockGetProfileIdFromApiKey.mockResolvedValueOnce({
				profileId: null,
	...
	describe('deduplication', () => {
		...
		it('should normalize URLs by removing only tracking params', async () => {
			...
			const linkedInJob = {
				...validJob,
				sourceUrl: 'https://www.linkedin.com/jobs/view/123456?utm_source=google&currentJobId=789'
			};
			...
			expect(mockInsertValues).toHaveBeenCalledWith(
				expect.objectContaining({
					source_url: 'https://www.linkedin.com/jobs/view/123456?currentJobId=789'
				})
			);
		});
	});
