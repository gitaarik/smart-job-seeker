...
export const POST: RequestHandler = async ({ locals, cookies, request }) => {
	...
	if (single) {
		...
		const match = allRows.find((r) => r.entity === entity && r.id === id && r.field === field);
		...
		if (!overwrite) {
			...
			const done = new Set(
				existing.map((e) => translationKey(e.entity_type, e.entity_id, e.field))
			);
			targets = targets.filter((r) => !done.has(translationKey(r.entity, r.id, r.field)));
		}
	}
