...
export const POST: RequestHandler = async ({ locals, request }) => {
	...
	const result = await shareCredential(platformCredentialId, user.id, sharedWithUserId);
	if (!result.success) {
...
export const DELETE: RequestHandler = async ({ locals, request }) => {
	...
	const removed = await unshareCredential(platformCredentialId, user.id, sharedWithUserId);
	if (!removed) {
