...
import { referenceReorderSchema, parseBody } from '$lib/server/validation/api-schemas';

export const PATCH: RequestHandler = async ({ request, locals }) => {
	const user = requireAuth(locals);

	const { profile_id, order } = parseBody(referenceReorderSchema, await request.json());
