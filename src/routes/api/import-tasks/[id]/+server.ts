...
import { parseBody, searchTaskUpdateSchema } from '$lib/server/validation/api-schemas';
...
export const PATCH: RequestHandler = async ({ params, locals, request }) => {
	...
	async function bindCredentialToProfile(credentialId: number): Promise<number> {
	...
	if (body.new_credential && searchTask.platform_id) {
		...
		if (body.platform_credential_id === null) {
			...
			if (searchTask.platform_id && cred.platform_id !== searchTask.platform_id) {
	...
	const finalPpId =
		data.platform_profile_id !== undefined
			? data.platform_profile_id
			: (searchTask.platform_profile_id ?? null);
	const finalDeviceId =
		data.sjsbrowser_api_key !== undefined
	...
	if (finalPpId !== null) {
		...
		if (credOwner && credOwner !== user.id) {
			...
			if (finalDeviceId === null) {
				throw error(400, 'Shared credentials require a device owned by the credential owner');
			}
			...
			if (key?.user_id !== credOwner) {
				throw error(400, "This credential can only be used with the owner's devices");
			}
		}
	}
