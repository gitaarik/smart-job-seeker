...
import { search_task_run_items, search_task_runs, search_tasks } from '$lib/server/db/schema';
...
export const POST: RequestHandler = async ({ params, locals }) => {
	...
	if ((user as { is_demo?: boolean }).is_demo) {
		...
		if (!cap.allowed) {
			throw error(429, `This demo is limited to ${cap.max} scrape runs (used ${cap.used}).`);
		}
	}
	...
	if (providerRequiresDevice(searchTask.browser_provider, config.browserProvider)) {
