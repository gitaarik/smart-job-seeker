...
export const POST: RequestHandler = async ({ params, locals, request }) => {
	...
	const apiKeyIdRaw = task.sjsbrowser_api_key != null ? String(task.sjsbrowser_api_key) : null;
	...
	try {
		const res = await fetch(`http://${tunnelHost}:${tunnelPort}/open-page/${device.apiKeyId}`, {
			method: 'POST',
		...
		if (!res.ok) {
			...
			throw error(res.status === 404 ? 404 : 502, data.error || 'Failed to open page');
		}
