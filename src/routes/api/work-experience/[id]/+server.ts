...
async function updateBasicInfo(id: number, data: Record<string, unknown>) {
  ...
  await db.update(work_experiences).set(updateData).where(
...
async function updateTechnologies(
  ...
  const techData = technologies
    ...
    .filter((t): t is { name: string; tags: string[] | null; sort: number } =>
      !!t.name
    )
    .map((t) => ({
      name: t.name,
...
async function updateProjects(
  ...
  const keepIds = incoming
    .map((p) => p.id)
    .filter((x): x is number => Number.isInteger(x));
  await db.delete(work_experience_projects).where(
    keepIds.length > 0
      ? and(
  ...
  for (let i = 0; i < incoming.length; i++) {
    ...
    await db.delete(work_experience_project_technologies).where(
      eq(
        work_experience_project_technologies.work_experience_project_id,
        row.id,
      ),
    );
