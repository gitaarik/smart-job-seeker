<script lang="ts">
	...
	const languageText = profile.languages
		.map(
			(lang: (typeof profile.languages)[number]) =>
...
<InfoSection title="Profile" icon={faUser}>
	<ul class="columns-2 gap-6">
		{#each profileData as item (item.label)}
			<li class="mb-4 flex break-inside-avoid items-start" aria-label={item.label}>
				<div class="mr-2 flex-shrink-0">
					<FontAwesomeIcon icon={item.icon} class="mt-1 h-4 w-4" title={item.label} />
				</div>
				<p>
					{#if item.href}
						<a
							href={item.href}
							class="text-slate hover:text-teal underline"
							target={item.target || ''}
							title={item.label}
						>
							{item.text}
