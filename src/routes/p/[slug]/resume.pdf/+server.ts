...
import { checkProfileAccess, getVersionSlugById } from '$lib/server/profile/access-control';
 ...
 */
function transformVersionToExportFormat(versionSlug: string, docType: 'Resume' | 'CV'): string {
	const formatted = versionSlug.replace(/-/g, ' ').replace(/\b\w/g, (l) => l.toUpperCase());
	return `${docType} (${formatted})`;
}

export const GET: RequestHandler = async ({ params, url, locals, getClientAddress, request }) => {
	const { slug } = params;
	const token = url.searchParams.get('t');
	const templateFilter = templateForStorage(url.searchParams.get('template'));
	const langParam = url.searchParams.get('lang');
	const localeFilter = isKnownLocale(langParam) && langParam !== BASE_LOCALE ? langParam : null;
	...
	if (effectiveVersion) {
		...
		if (!exportWithFile) {
			const transformedFormat = transformVersionToExportFormat(effectiveVersion, 'Resume');
			exportWithFile = await getLatestExportWithFile({
				profileId: profile.id,
				exportType: 'resume',
				fileType: 'pdf',
	...
	if (!exportWithFile) {
		const versionMsg = effectiveVersion ? ` for version "${effectiveVersion}"` : '';
		throw error(
			404,
	...
	const filename = effectiveVersion
		? `${slug}-resume-${effectiveVersion}${localeSuffix}.pdf`
		: `${slug}-resume${localeSuffix}.pdf`;
