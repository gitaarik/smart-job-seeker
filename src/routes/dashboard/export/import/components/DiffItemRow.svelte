<script lang="ts">
  ...
  const changedFields = $derived(
    fieldDiffs?.filter((d) => d.changed) ?? [],
  );

  const hasNestedChanges = $derived(
    nestedDiffs && nestedDiffs.length > 0,
  );

  const hasDetails = $derived(
    changedFields.length > 0 || hasNestedChanges,
  );
</script>
...
{#if type !== "unchanged"}
  <div class={borderClass}>
    ...
    >
      <div class="flex items-center gap-3 flex-1 p-3 sm:p-4">
        ...
        <FontAwesomeIcon icon={typeIcon} class="w-3.5 h-3.5 {typeColor}" />

        <div class="min-w-0">
          <div class="font-semibold text-sm text-[var(--dash-text)]">
            {title}
          </div>
          {#if subtitle}
        ...
        {#if type === "modified" && changedFields.length > 0}
          <span
            class="text-xs text-[var(--dash-text-muted)] ml-auto mr-2"
          >
            {changedFields.length} field{changedFields.length !== 1
              ? "s"
              : ""} changed
          </span>
        {/if}
    ...
    {#if isExpanded && hasDetails}
      <div class="px-3 sm:px-4 pb-4 space-y-1 ml-7">
              ...
              {#if nested.added.length > 0}
                <div class="space-y-1">
                  {#each nested.added as item, i}
                    <div
                      class="flex items-center gap-2 py-1 px-2 rounded bg-green-50 text-sm"
                    >
                      <label class="flex-shrink-0">
                        <input
                          type="checkbox"
              ...
              {#if nested.removed.length > 0}
                <div class="space-y-1 mt-1">
                  {#each nested.removed as item, i}
                    <div
                      class="flex items-center gap-2 py-1 px-2 rounded bg-red-50 text-sm"
                    >
                      <label class="flex-shrink-0">
                        <input
                          type="checkbox"
                      ...
                      />
                      <span
                        class="text-[var(--dash-text-muted)] line-through"
                        >{item}</span
                      >
                    </div>
                  {/each}
