...
function reconstructNew(segments: DiffSegment[]): string {
  return segments.filter((s) => s.type !== "removed").map((s) => s.text).join(
    "",
  );
}
...
describe("computeDiff", () => {
  ...
  it("marks purely added words", () => {
    ...
    expect(segs.some((s) => s.type === "added" && s.text.includes("there")))
      .toBe(true);
  ...
  it("marks purely removed words", () => {
    ...
    expect(segs.some((s) => s.type === "removed" && s.text.includes("there")))
      .toBe(true);
  ...
  it("handles a replacement as removed + added", () => {
    ...
    expect(segs.some((s) => s.type === "removed" && s.text.includes("quick")))
      .toBe(true);
    expect(segs.some((s) => s.type === "added" && s.text.includes("slow")))
      .toBe(true);
...
describe("isSmallDiff", () => {
  ...
  it("is false when most of the text changed", () => {
    const segs = computeDiff(
      ...
      "an entirely fresh rewrite here",
    );
