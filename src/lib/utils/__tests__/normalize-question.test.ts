...
describe('normalizeQuestion', () => {
	...
	it('keeps genuinely different questions distinct', () => {
		expect(normalizeQuestion('Why do you want to work here?')).not.toBe(
			normalizeQuestion('What draws you to this role?')
		);
	});

	it('does not strip internal or non-trailing punctuation', () => {
		// Only trailing sentence punctuation is removed — mid-string stays.
		expect(normalizeQuestion('Tell us about a project. What went wrong?')).toBe(
			'tell us about a project. what went wrong'
		);
	});
