...
import { htmlToMarkdown, isLikelyHtml } from '$lib/utils/html-to-markdown';
...
describe('htmlToMarkdown', () => {
	...
	it('converts headings and paragraphs', () => {
		expect(htmlToMarkdown('<h1>Me</h1><p>Some intro.</p>')).toBe('# Me\n\nSome intro.');
	...
	it('converts inline bold, italic, and links', () => {
		expect(htmlToMarkdown('<p>a <strong>bold</strong> word</p>')).toBe('a **bold** word');
		expect(htmlToMarkdown('<p>an <em>italic</em> word</p>')).toBe('an *italic* word');
		expect(htmlToMarkdown('<p>see <a href="https://x.io">the site</a></p>')).toBe(
			'see [the site](https://x.io)'
		);
	});

	it('decodes entities', () => {
		expect(htmlToMarkdown('<p>Spain &amp; Portugal</p>')).toBe('Spain & Portugal');
	});
