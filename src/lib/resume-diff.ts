...
function diffEducation(current: Education[], incoming: Education[]): ItemDiff<Education>[] {
	const { matched, added, removed } = matchItems(
		current,
		incoming,
...
function diffSkillCategory(current: SkillCategory, incoming: SkillCategory): SkillCategoryDiff {
	const fieldDiffs = [diffField('name', 'Category Name', current.name, incoming.name)];

	// Match skills by name within category
	const { matched, added, removed } = matchItems(current.skills, incoming.skills, (s) => s.name);
	...
	const hasChanges =
		fieldDiffs.some((d) => d.changed) || skillDiffs.some((d) => d.type !== 'unchanged');
...
function diffSkills(current: SkillCategory[], incoming: SkillCategory[]): SkillCategoryDiff[] {
	const { matched, added, removed } = matchItems(current, incoming, (c) => c.name);
...
function diffLanguages(current: Language[], incoming: Language[]): ItemDiff<Language>[] {
	const { matched, added, removed } = matchItems(current, incoming, (l) => l.name);
...
function diffProjects(current: SideProject[], incoming: SideProject[]): ItemDiff<SideProject>[] {
	const { matched, added, removed } = matchItems(current, incoming, (p) => p.name);
...
function diffReferences(current: Reference[], incoming: Reference[]): ItemDiff<Reference>[] {
	const { matched, added, removed } = matchItems(current, incoming, (r) => r.author);
...
export function diffResumeData(current: ResumeData, incoming: ResumeData): ResumeDataDiff {
	...
	const work = hasData(incoming.work) ? diffWork(current.work ?? [], incoming.work) : [];
	const education = hasData(incoming.education)
		? diffEducation(current.education ?? [], incoming.education)
		: [];
	const skills = hasData(incoming.skills) ? diffSkills(current.skills ?? [], incoming.skills) : [];
...
export function countEnabledChanges(diff: ResumeDataDiff): number {
	...
	for (const section of [
		...
		diff.certificates,
		diff.references
	] as { type: ItemDiffType; enabled: boolean }[][]) {
