...
export async function generateInterviewQuestionAnswer(
  ...
  try {
    ...
    const question = await prisma.application_interview_questions.findUnique({
      where: { id: questionId },
      include: {
        applications: {
          include: {
...
\${jobDescription}`;
    ...
    const aiChat = await prisma.ai_chat.create({
      data: {
        profile: profileId,
    ...
    await prisma.application_interview_questions.update({
      where: { id: questionId },
