...
import { api_keys, type DemoLinks, demo_link_devices, demo_links } from '$lib/server/db/schema';
...
export async function createDemoLink(params: CreateDemoLinkParams): Promise<DemoLinks> {
...
export async function revokeDemoLink(linkId: number, createdBy: string): Promise<void> {
	await db
		.update(demo_links)
		.set({ status: 'revoked' })
...
export async function listDemoLinks(createdBy: string): Promise<DemoLinkListItem[]> {
	...
	const grants = await db.query.demo_link_devices.findMany({
		where: inArray(
			demo_link_devices.demo_link_id,
			links.map((l) => l.id)
		)
	});
