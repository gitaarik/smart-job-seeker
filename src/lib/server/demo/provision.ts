...
async function getTemplateProfileId(): Promise<number> {
	...
	if (!template) {
		throw new Error('No demo-template account found. Run `npm run demo:seed-template` first.');
	}
...
async function wireDemoCredentials(
	...
	const platformIds = Array.from(
		new Set(tasks.map((t) => t.platform_id).filter((id): id is number => id != null))
	);
 ...
 */
export async function provisionDemoUser(link: DemoLinks): Promise<DemoCredentials> {
	...
	await db.update(demo_links).set({ demo_user_id: creds.userId }).where(eq(demo_links.id, link.id));
	return creds;
}
 ...
 */
export async function resumeDemoUser(link: DemoLinks): Promise<DemoCredentials | null> {
