...
export async function removeContact(contactId: number, userId: string): Promise<boolean> {
	...
	const { revokeAllSharesBetweenContacts } = await import('$lib/server/credential-shares');
	await revokeAllSharesBetweenContacts(contact.requester_id, contact.recipient_id);
 ...
 */
export async function ensureAcceptedContact(userA: string, userB: string): Promise<void> {
