...
export async function createFollowupAiChat(
  ...
  try {
    // Step 1: Fetch parent ai_chat record
    const parent = await db.ai_chat.findUnique({
    ...
    // Step 2: Verify parent has a response
    if (!parent.response) {
      return {
        success: false,
        message:
    ...
    // Step 3: Prepare custom variables for the "followup" template
    ...
    // Step 4: Call createAndGenerateAiChat with the "followup" template
    ...
    // Step 5: Auto-update parent records (application_letters/questions if linked)
    const newAiChatId = result.aiChat.id;

    // Check and update application_letters
    const linkedLetters = await db.application_letters.findMany({
    ...
    // Check and update application_questions
    const linkedQuestions = await db.application_questions.findMany({
      where: { ai_chat: parentAiChatId },
      select: { id: true },
    });
    ...
    return {
      success: false,
      message: `Error creating follow-up ai_chat: ${errorMessage}`,
    };
  }
}
