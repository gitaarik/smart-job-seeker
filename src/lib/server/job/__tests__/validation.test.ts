...
describe('validateJobImport', () => {
	...
	it('rejects missing title', () => {
		expect(() => validateJobImport({ company: 'X', sourceUrl: 'https://x.com' })).toThrow(ZodError);
	});

	it('rejects empty title', () => {
		expect(() => validateJobImport({ ...validJob, title: '' })).toThrow(ZodError);
	});

	it('rejects missing company', () => {
		expect(() => validateJobImport({ title: 'X', sourceUrl: 'https://x.com' })).toThrow(ZodError);
	});

	it('rejects missing sourceUrl', () => {
		expect(() => validateJobImport({ title: 'X', company: 'Y' })).toThrow(ZodError);
	});

	it('rejects invalid sourceUrl', () => {
		expect(() => validateJobImport({ ...validJob, sourceUrl: 'not-a-url' })).toThrow(ZodError);
	});

	// Length limits
	it('rejects title > 255 chars', () => {
		expect(() => validateJobImport({ ...validJob, title: 'x'.repeat(256) })).toThrow(ZodError);
	});
	...
	it('rejects invalid salaryPeriod', () => {
		expect(() => validateJobImport({ ...validJob, salaryPeriod: 'biweekly' })).toThrow(ZodError);
	});

	it('rejects invalid remote option', () => {
		expect(() => validateJobImport({ ...validJob, remote: 'partially' })).toThrow(ZodError);
	});

	it('rejects invalid jobType', () => {
		expect(() => validateJobImport({ ...validJob, jobType: 'volunteer' })).toThrow(ZodError);
	});
	...
	it('rejects negative salaryMin', () => {
		expect(() => validateJobImport({ ...validJob, salaryMin: -1 })).toThrow(ZodError);
	});

	it('rejects non-integer salaryMax', () => {
		expect(() => validateJobImport({ ...validJob, salaryMax: 50000.5 })).toThrow(ZodError);
	});
