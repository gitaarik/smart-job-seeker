...
describe('expandProfileSkills', () => {
	...
	it('embeds + persists skills not yet in the vocabulary', async () => {
		...
		expect(mockValues).toHaveBeenCalledWith(
			expect.arrayContaining([expect.objectContaining({ skill: 'react', label: 'React' })])
		);
	});
	...
	it('still persists the valid vectors in a partially-empty batch', async () => {
		...
		await expect(backfillSkillVocabulary(['React', 'BadSkill'])).rejects.toThrow(/empty vectors/);
		expect(mockValues).toHaveBeenCalledWith(
			expect.arrayContaining([expect.objectContaining({ skill: 'react' })])
		);
		expect(mockValues).not.toHaveBeenCalledWith(
			expect.arrayContaining([expect.objectContaining({ skill: 'badskill' })])
		);
	});
