 ...
 */
export function buildScoreFilter(minScore: string): {
	filter: SQL;
	isActive: boolean;
	isUnmatched: boolean;
} {
...
export async function listJobs(
	...
	if (filters.platform) {
		const platformIds = filters.platform
			.split(',')
	...
	if (filters.importedBy) {
		const values = filters.importedBy
			.split(',')
			.map((v) => v.trim())
			.filter(Boolean);
    ...
    `);
		...
		if (jobIds.length > 0) {
			...
			jobs = jobIds.map((id) => jobById.get(id)).filter(Boolean) as typeof fullJobs;
		}
