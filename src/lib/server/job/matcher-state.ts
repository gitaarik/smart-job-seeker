 ...
 */
export async function setMatcherState(profileId: number, state: MatcherState): Promise<void> {
	const redis = getRedisClient();
	await redis.set(matcherStateKey(profileId), JSON.stringify(state), 'EX', MATCHER_STATE_TTL);
}

/**
 * Read matcher state from Redis for a specific profile (called from app API)
 */
export async function getMatcherState(profileId: number): Promise<MatcherState | null> {
	const redis = getRedisClient();
