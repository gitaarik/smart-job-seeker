...
export interface AppConfig {
	...
	llmProvider: 'groq' | 'gemini' | 'openai' | 'deepseek' | 'cerebras';
...
function getModelForProvider(provider: string): string {
	...
	const hardcodedDefault = hardcodedDefaults[provider] || hardcodedDefaults.groq;
	const providerEnvVar = providerEnvVars[provider];

	// Priority: provider-specific → hardcoded default
	return providerEnvVar ? getEnv(providerEnvVar, hardcodedDefault) : hardcodedDefault;
}
...
function loadConfig(): AppConfig {
	...
	const config = {
		...
		llmTranslateModel:
			getEnv('SJS_LLM_TRANSLATE_MODEL', '') ||
		...
		embeddingWorkingDimensions: parseInt(
			getEnv('SJS_EMBEDDING_WORKING_DIMENSIONS', String(embeddingDefaults.workingDimensions)),
			10
		),
		...
		retryInitialDelay: parseInt(getEnv('SJS_LLM_RETRY_INITIAL_DELAY', '1000'), 10),
...
export function validateConfig(): void {
	...
	const required: (keyof AppConfig)[] = ['databaseUrl'];
