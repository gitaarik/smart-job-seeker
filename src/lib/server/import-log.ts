...
function detectDocType(data: ResumeData): "partial" | "full" {
  const present = [data.work, data.education, data.skills, data.languages, data.projects, data.references]
...
function summariseChanges(payload: DiffApplyPayload) {
  ...
  for (const key of ["work", "education", "skills", "languages", "projects", "references"] as const) {
