...
export type SkipReason =
	'traversal' | 'symlink' | 'oversize' | 'binary' | 'ignored' | 'nested-archive' | 'too-deep';
...
const NESTED_ARCHIVE_EXTS = new Set(['zip', 'tar', 'gz', 'tgz', 'bz2', 'rar', '7z', 'xz', 'zst']);
...
export async function safeUnzip(
	...
	try {
		...
		throw new SafeUnzipError(`Not a readable ZIP archive: ${(err as Error).message}`);
	}
