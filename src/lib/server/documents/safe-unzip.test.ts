...
describe('safeUnzip', () => {
	it('extracts allowlisted source files and skips binaries', async () => {
		...
		expect(text(res.entries.find((e) => e.path === 'src/index.ts')!.bytes)).toBe(
			'export const x = 1;'
		);
	...
	it('throws when cumulative uncompressed size exceeds the cap (zip bomb)', async () => {
		...
		await expect(safeUnzip(buf, baseLimits({ maxTotalUncompressed: 100 }))).rejects.toBeInstanceOf(
			SafeUnzipError
		);
	});
	...
	it('throws SafeUnzipError on non-zip input', async () => {
		await expect(safeUnzip(Buffer.from('not a zip'), baseLimits())).rejects.toBeInstanceOf(
			SafeUnzipError
		);
	});
});
