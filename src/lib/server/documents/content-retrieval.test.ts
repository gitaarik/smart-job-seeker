...
describe('scoreUnitAgainstQuery', () => {
	it('scores zero for an empty query', () => {
		expect(
			scoreUnitAgainstQuery(
				{ title: 'Django migration', keywords: [], text: 'x' },
				{
					text: ''
				}
			)
	...
	it('weights a title-token match double a body-token match', () => {
		...
		expect(scoreUnitAgainstQuery({ title: 'Kubernetes rollout', keywords: [], text: '' }, q)).toBe(
			2
		);
		expect(
			scoreUnitAgainstQuery(
				{ title: 'Migration notes', keywords: [], text: 'we used kubernetes here' },
				q
			)
		).toBe(1);
	});
	...
	it('ignores stopwords and short tokens', () => {
		expect(
			scoreUnitAgainstQuery(
				{ title: 'the and for', keywords: [], text: '' },
				{
					text: 'the and for'
				}
			)
...
describe('buildStoryUnit', () => {
	...
	it('skips a story with an empty STAR body', () => {
		expect(
			buildStoryUnit(
				row({ situation: null, task: null, action: null, result: null, reflection: null })
			)
		).toBeNull();
	});
