...
const STOPWORDS = new Set([
  ...
  "were", "are", "you", "your", "our", "their", "its", "into", "about", "over",
  "then", "than", "them", "they", "when", "what", "which", "who", "how", "why",
  "not", "but", "all", "any", "can", "will", "would", "should", "could", "been",
  "using", "used", "use", "via", "per", "out", "off", "some", "more", "most",
]);

/** Significant lowercase tokens (len ≥ 3, non-stopword); keeps + # . for tech. */
function tokens(s: string): Set<string> {
  const out = new Set<string>();
  for (const raw of s.toLowerCase().replace(/[^a-z0-9+#. ]/g, " ").split(/\s+/)) {
...
export function scoreUnitAgainstQuery(
  ...
  const queryTokens = tokens(
    [query.text, ...(query.skills ?? [])].filter(Boolean).join(" "),
  );
  ...
  let score = intersectSize(queryTokens, titleTokens) * 2 +
    intersectSize(queryTokens, bodyTokens);
  ...
  for (const raw of query.skills ?? []) {
    ...
    if (
      kw.some((k) =>
        k === s || (s.length >= 3 && k.includes(s)) ||
        (k.length >= 3 && s.includes(k))
      )
    ) {
      score += 2;
    }
...
export async function relevantUnits(
  ...
  const queryText = [query.text, ...(query.skills ?? [])]
    .filter(Boolean)
    .join("\n");
...
const PLACEHOLDER_STORY_TITLES = new Set([
  "new story",
  "untitled",
  "untitled story",
]);
...
async function loadStoryUnits(profileId: number): Promise<RankableUnit[]> {
  ...
  const units = rows
    .map(buildStoryUnit)
    .filter((u): u is RankableUnit => u !== null);
