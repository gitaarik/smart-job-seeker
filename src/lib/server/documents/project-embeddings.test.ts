...
describe("projectKey", () => {
  it("namespaces by kind so ids from the two tables never collide", () => {
    ...
    expect(projectKey("work_experience_project", 5)).toBe(
      "work_experience_project:5",
    );
    expect(projectKey("side_project", 5)).not.toBe(
      projectKey("work_experience_project", 5),
    );
  });
});
