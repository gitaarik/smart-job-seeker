	...
	for (let attempt = 1; attempt <= opts.maxAttempts; attempt++) {
		try {
			...
			if (attempt === opts.maxAttempts) {
				errorTracker.logError(`All retry attempts exhausted (${opts.maxAttempts})`, lastError, {
					operation: 'withRetry'
				});
				throw lastError;
			}
			...
			const delay =
				retryAfterMs !== undefined
					...
					: calculateDelay(attempt, opts.initialDelay, opts.maxDelay, opts.backoffMultiplier);
...
export function isRetryableError(error: Error): boolean {
	...
	if ('status' in error && (error as { status?: unknown }).status === 429) {
		...
		return typeof retryAfter === 'number' && retryAfter <= RATE_LIMIT_RETRY_MAX_SECONDS;
	}
