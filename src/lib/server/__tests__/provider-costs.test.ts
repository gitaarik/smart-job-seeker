...
describe('estimateProviderCostUsd', () => {
	...
	it('discounts input the provider served from cache', () => {
		...
		expect(estimateProviderCostUsd(...PRO, 25_000, 400, 14_000)).toBeCloseTo(0.0195, 6);
	});
	...
	it('clamps a cached count larger than the input count', () => {
		...
		expect(clamped).toBeCloseTo(estimateProviderCostUsd(...PRO, 1_000, 100, 1_000)!, 9);
		expect(clamped).toBeGreaterThan(0);
	});

	it('ignores a negative cached count', () => {
		expect(estimateProviderCostUsd(...PRO, 1_000, 100, -50)).toBeCloseTo(
			estimateProviderCostUsd(...PRO, 1_000, 100, 0)!,
			9
		);
	});
	...
	it('switches to long-context rates past the threshold', () => {
		...
		expect(over).toBeCloseTo(200_001 * 2.5e-6 + 1_000 * 15e-6, 6);
	});
	...
	it('prices a model with no long-context tier at one rate throughout', () => {
		...
		expect(estimateProviderCostUsd(...key, 400_000, 100)).toBeCloseTo(
			400_000 * 0.15e-6 + 100 * 0.6e-6,
			6
		);
	});
});
...
describe('an unpriced model', () => {
	it('returns null and says so, instead of silently recording null', () => {
		...
		try {
			expect(estimateProviderCostUsd('acme', 'wonder-model-1', 100, 10)).toBeNull();
