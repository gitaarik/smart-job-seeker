...
describe('applyInviteGrants', () => {
	...
	it('writes an admin-granted subscription ending planMonths out', async () => {
		...
		expect(Math.abs(row.current_period_end.getTime() - expected.getTime())).toBeLessThan(60_000);
	});
	...
	it('treats an already-shared device as success', async () => {
		...
		expect(await applyInviteGrants('bob', { inviterId: 'admin', deviceIds: [3] })).toEqual([]);
	});
