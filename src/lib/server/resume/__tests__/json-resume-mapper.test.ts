...
describe('validateJsonResume', () => {
	...
	it('should throw error for non-object input', () => {
		expect(() => validateJsonResume(null)).toThrow('Invalid JSON Resume: data must be an object');
		expect(() => validateJsonResume('string')).toThrow(
			'Invalid JSON Resume: data must be an object'
		);
		expect(() => validateJsonResume(123)).toThrow('Invalid JSON Resume: data must be an object');
	});
...
describe('mapJsonResumeToInternal', () => {
	...
	it('should map complete basics information', async () => {
		...
		expect(result.basics.stackoverflow).toBe('https://stackoverflow.com/users/123456');
	});
			...
			work: [
				{
					...
					highlights: ['Improved performance by 50%', 'Mentored 5 junior developers']
				}
			]
		};
	...
	it('should map references correctly', async () => {
		...
		expect(result.references?.[0].text).toBe('John is an excellent developer and team player.');
	});
	...
	it('should handle case-insensitive social network matching', async () => {
		...
		expect(result.basics.stackoverflow).toBe('https://stackoverflow.com/users/123');
	});
