 ...
 */
export function mapJsonResumeToInternal(jsonResume: JsonResumeSchema): ResumeData {
	// Validate basics
	...
	const linkedin = jsonResume.basics.profiles?.find(
		(p) => p.network?.toLowerCase() === 'linkedin'
	)?.url;
	const github = jsonResume.basics.profiles?.find(
		(p) => p.network?.toLowerCase() === 'github'
	)?.url;
	const stackoverflow = jsonResume.basics.profiles?.find(
		(p) =>
			p.network?.toLowerCase() === 'stackoverflow' || p.network?.toLowerCase() === 'stack overflow'
	)?.url;
...
function mapLanguageProficiency(
	fluency?: string
): 'native' | 'fluent' | 'proficient' | 'conversational' | 'basic' | undefined {
	...
	if (normalized.includes('fluent') || normalized.includes('full professional')) {
		return 'fluent';
	}
	if (normalized.includes('proficient') || normalized.includes('professional')) {
		return 'proficient';
	}
	if (
