...
async function getMaxSort(table: any, whereCol: any, whereVal: any): Promise<number> {
  const [result] = await dbDirect.select({ value: max(table.sort) }).from(table).where(eq(whereCol, whereVal));
...
export async function applyDiffToProfile(
  ...
  if (payload.work) {
    for (const w of payload.work.added ?? []) {
      const [created] = await dbDirect.insert(work_experiences).values({
        profile_id: profileId, name: w.name || "", position: w.position || "",
        location: w.location || "", description: "", summary: w.summary || "",
        status: "draft", start_date: parseDate(w.startDate), end_date: parseDate(w.endDate),
        website: w.website || null,
      }).returning();

      let sort = 1;
      for (const ach of w.achievements ?? []) {
        await dbDirect.insert(work_experience_achievements).values({ work_experience_id: created.id, description: ach, status: "draft", sort: sort++ });
      }
      sort = 1;
      for (const tech of w.technologies ?? []) {
        await dbDirect.insert(work_experience_technologies).values({ work_experience_id: created.id, name: tech, status: "draft", sort: sort++ });
      }
    }
    ...
    for (const mod of payload.work.modified ?? []) {
      ...
      const existing = await dbDirect.query.work_experiences.findFirst({
        where: and(eq(work_experiences.profile_id, profileId), eq(work_experiences.name, company), eq(work_experiences.position, position)),
        columns: { id: true },
      });
      ...
      if (Object.keys(updateData).length > 0) {
        await dbDirect.update(work_experiences).set(updateData).where(eq(work_experiences.id, existing.id));
      }

      if (mod.addAchievements?.length) {
        let sort = (await getMaxSort(work_experience_achievements, work_experience_achievements.work_experience_id, existing.id)) + 1;
        for (const ach of mod.addAchievements) {
          await dbDirect.insert(work_experience_achievements).values({ work_experience_id: existing.id, description: ach, status: "draft", sort: sort++ });
        }
      }
      if (mod.removeAchievements?.length) {
        for (const ach of mod.removeAchievements) {
          await dbDirect.delete(work_experience_achievements).where(and(eq(work_experience_achievements.work_experience_id, existing.id), eq(work_experience_achievements.description, ach)));
        }
      }
      if (mod.addTechnologies?.length) {
        let sort = (await getMaxSort(work_experience_technologies, work_experience_technologies.work_experience_id, existing.id)) + 1;
        for (const tech of mod.addTechnologies) {
          await dbDirect.insert(work_experience_technologies).values({ work_experience_id: existing.id, name: tech, status: "draft", sort: sort++ });
        }
      }
      if (mod.removeTechnologies?.length) {
        for (const tech of mod.removeTechnologies) {
          await dbDirect.delete(work_experience_technologies).where(and(eq(work_experience_technologies.work_experience_id, existing.id), eq(work_experience_technologies.name, tech)));
        }
      }
    }
    ...
    for (const key of payload.work.removed ?? []) {
      ...
      if (!existing) continue;
      await dbDirect.delete(work_experience_achievements).where(eq(work_experience_achievements.work_experience_id, existing.id));
      await dbDirect.delete(work_experience_technologies).where(eq(work_experience_technologies.work_experience_id, existing.id));
      const projects = await dbDirect.query.work_experience_projects.findMany({ where: eq(work_experience_projects.work_experience_id, existing.id), columns: { id: true } });
      ...
      await dbDirect.delete(work_experience_projects).where(eq(work_experience_projects.work_experience_id, existing.id));
  ...
  if (payload.education) {
    for (const e of payload.education.added ?? []) {
      await dbDirect.insert(education).values({ profile_id: profileId, status: "draft", institution: e.institution || null, area: e.area || null, study_type: e.studyType || null, location: e.location || null, url: e.url || null, start_date: parseDate(e.startDate), end_date: parseDate(e.endDate), graduation_year: e.graduationYear ?? null, summary: e.summary || null });
    }
    ...
    for (const mod of payload.education.modified ?? []) {
      ...
      const existing = await dbDirect.query.education.findFirst({
        where: and(eq(education.profile_id, profileId), eq(education.institution, institution), area ? eq(education.area, area) : undefined as any),
        columns: { id: true },
      });
      if (!existing) continue;
      const updateData: Record<string, unknown> = {};
    ...
    for (const key of payload.education.removed ?? []) {
      ...
      await dbDirect.delete(education).where(and(eq(education.profile_id, profileId), eq(education.institution, institution)));
    }
  }
  ...
  if (payload.skills) {
    for (const cat of payload.skills.added ?? []) {
      const [created] = await dbDirect.insert(tech_skill_categories).values({ profile_id: profileId, name: cat.name || null, status: "draft" }).returning();
      let sort = 1;
      for (const skill of cat.skills ?? []) {
        await dbDirect.insert(tech_skills).values({ category_id: created.id, name: skill.name || null, level: skill.level || null, years_experience: skill.yearsExperience ?? null, status: "draft", sort: sort++ });
      }
    }

    for (const mod of payload.skills.modified ?? []) {
      const existing = await dbDirect.query.tech_skill_categories.findFirst({
        where: and(eq(tech_skill_categories.profile_id, profileId), eq(tech_skill_categories.name, mod.matchKey)),
        columns: { id: true },
      });
      if (!existing) continue;
      ...
      if (mod.addSkills?.length) {
        ...
        for (const skill of mod.addSkills) {
          await dbDirect.insert(tech_skills).values({ category_id: existing.id, name: skill.name || null, level: skill.level || null, years_experience: skill.yearsExperience ?? null, status: "draft", sort: sort++ });
        }
      }
      if (mod.removeSkills?.length) {
        for (const name of mod.removeSkills) {
          await dbDirect.delete(tech_skills).where(and(eq(tech_skills.category_id, existing.id), eq(tech_skills.name, name)));
        }
      }
      if (mod.modifySkills?.length) {
        for (const skillMod of mod.modifySkills) {
          const skill = await dbDirect.query.tech_skills.findFirst({
            where: and(eq(tech_skills.category_id, existing.id), eq(tech_skills.name, skillMod.name)),
            columns: { id: true },
          });
          if (!skill) continue;
          const updateData: Record<string, unknown> = {};
          if (skillMod.fields.level !== undefined) updateData.level = skillMod.fields.level || null;
          if (skillMod.fields.yearsExperience !== undefined) updateData.years_experience = skillMod.fields.yearsExperience ?? null;
          if (Object.keys(updateData).length > 0) {
    ...
    for (const name of payload.skills.removed ?? []) {
      const existing = await dbDirect.query.tech_skill_categories.findFirst({
        where: and(eq(tech_skill_categories.profile_id, profileId), eq(tech_skill_categories.name, name)),
        columns: { id: true },
      });
      if (!existing) continue;
  ...
  if (payload.languages) {
    for (const l of payload.languages.added ?? []) {
      await dbDirect.insert(languages).values({ profile_id: profileId, status: "draft", name: l.name || null, language_code: l.languageCode || null, proficiency: l.proficiency || null });
    }
    for (const mod of payload.languages.modified ?? []) {
      const existing = await dbDirect.query.languages.findFirst({ where: and(eq(languages.profile_id, profileId), eq(languages.name, mod.matchKey)), columns: { id: true } });
      if (!existing) continue;
      const updateData: Record<string, unknown> = {};
    ...
    for (const name of payload.languages.removed ?? []) {
      await dbDirect.delete(languages).where(and(eq(languages.profile_id, profileId), eq(languages.name, name)));
    }
  }
  ...
  if (payload.projects) {
    for (const p of payload.projects.added ?? []) {
      ...
      for (const tech of p.technologies ?? []) {
        await dbDirect.insert(side_project_technologies).values({ side_project_id: created.id, name: tech, sort: sort++ });
      }
    }

    for (const mod of payload.projects.modified ?? []) {
      const existing = await dbDirect.query.side_projects.findFirst({ where: and(eq(side_projects.profile_id, profileId), eq(side_projects.name, mod.matchKey)), columns: { id: true } });
      if (!existing) continue;
      const updateData: Record<string, unknown> = {};
      ...
      if (Object.keys(updateData).length > 0) {
        await dbDirect.update(side_projects).set(updateData).where(eq(side_projects.id, existing.id));
      }
      if (mod.addAchievements?.length) {
        let sort = (await getMaxSort(side_project_achievements, side_project_achievements.side_project_id, existing.id)) + 1;
        for (const ach of mod.addAchievements) {
          await dbDirect.insert(side_project_achievements).values({ side_project_id: existing.id, description: ach, sort: sort++ });
        }
      }
      if (mod.removeAchievements?.length) {
        for (const ach of mod.removeAchievements) {
          await dbDirect.delete(side_project_achievements).where(and(eq(side_project_achievements.side_project_id, existing.id), eq(side_project_achievements.description, ach)));
        }
      }
      if (mod.addTechnologies?.length) {
        let sort = (await getMaxSort(side_project_technologies, side_project_technologies.side_project_id, existing.id)) + 1;
        for (const tech of mod.addTechnologies) {
          await dbDirect.insert(side_project_technologies).values({ side_project_id: existing.id, name: tech, sort: sort++ });
        }
      }
      if (mod.removeTechnologies?.length) {
        for (const tech of mod.removeTechnologies) {
          await dbDirect.delete(side_project_technologies).where(and(eq(side_project_technologies.side_project_id, existing.id), eq(side_project_technologies.name, tech)));
        }
      }
    }

    for (const name of payload.projects.removed ?? []) {
      const existing = await dbDirect.query.side_projects.findFirst({ where: and(eq(side_projects.profile_id, profileId), eq(side_projects.name, name)), columns: { id: true } });
      if (!existing) continue;
      await dbDirect.delete(side_project_achievements).where(eq(side_project_achievements.side_project_id, existing.id));
      await dbDirect.delete(side_project_technologies).where(eq(side_project_technologies.side_project_id, existing.id));
  ...
  if (payload.references) {
    for (const r of payload.references.added ?? []) {
      await dbDirect.insert(references).values({ profile_id: profileId, status: "draft", author: r.author || "", author_position: r.authorPosition || null, text: r.text || null });
    }
    for (const mod of payload.references.modified ?? []) {
      const existing = await dbDirect.query.references.findFirst({ where: and(eq(references.profile_id, profileId), eq(references.author, mod.matchKey)), columns: { id: true } });
      if (!existing) continue;
      const updateData: Record<string, unknown> = {};
    ...
    for (const author of payload.references.removed ?? []) {
      await dbDirect.delete(references).where(and(eq(references.profile_id, profileId), eq(references.author, author)));
    }
  }
}
