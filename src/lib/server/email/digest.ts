		...
		.map((job) => {
			...
			if (job.work_location) {
				for (const loc of job.work_location)
					pills.push(pillHtml(formatWorkLocation(loc), '#ede9fe', '#6d28d9'));
			}
			if (job.job_types) {
				for (const t of job.job_types) pills.push(pillHtml(formatJobType(t), '#dbeafe', '#1d4ed8'));
			}
			if (job.experience_levels) {
				for (const e of job.experience_levels)
					pills.push(pillHtml(formatExperienceLevel(e), '#fef3c7', '#92400e'));
			}

			const matchedSet = new Set((job.matched_skills ?? []).map((s) => s.toLowerCase()));

			const requiredSkillPills = (job.skills_required ?? [])
				.slice(0, 10)
			...
			const preferredSkillPills = (job.skills_preferred ?? [])
				.slice(0, 10)
				.map((s) => skillPillHtml(s, matchedSet.has(s.toLowerCase())))
				.join('');
			const morePreferred =
				(job.skills_preferred ?? []).length > 10
					? `<span style="font-size: 11px; color: #9ca3af; margin-left: 2px;">+${(job.skills_preferred?.length ?? 0) - 10} more</span>`
					: '';

			const description = job.job_description
				? escapeHtml(job.job_description.slice(0, 300)) +
