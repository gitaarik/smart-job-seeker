...
const CODE_PATTERNS = [
	...
	/^(?:code|pin|otp)\s*[:\s]+(?<code>\d{4,8})\s*$/im,
...
${emailContent}`;
			...
			[
				{
					role: 'system',
					content:
						'You extract verification codes and links from emails. Respond with only valid JSON, no explanation.'
				},
		...
		console.warn(
			'[verification-parser] LLM fallback failed:',
...
function extractFromHtml(html: string, subject: string | null): ParsedVerification | null {
	...
	let verificationLink = links.find(
		(url) => isValidUrl(url) && isVerificationUrl(url) && !isExcludedUrl(url)
	);
