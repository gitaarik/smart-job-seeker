...
export async function checkDeviceRateBudget(params: {
  ...
  if (deviceRuns.length >= DEVICE_MAX_RUNS_PER_DAY) {
    return {
      allowed: false,
      error:
        `This device has hit its daily scrape limit (${DEVICE_MAX_RUNS_PER_DAY}/day). Try again later.`,
    };
  }
  ...
  if (
    lastRunAt &&
    Date.now() - new Date(lastRunAt).getTime() < DEVICE_MIN_RUN_SPACING_MS
  ) {
    return {
      allowed: false,
      error:
        "This device ran a scrape moments ago. Please wait a few minutes between runs.",
    };
  }
  ...
  if (params.isShared) {
    ...
    if (shareeRuns.length >= SHAREE_MAX_RUNS_PER_DAY) {
      return {
        allowed: false,
        error:
          `You've reached your daily limit on this shared device (${SHAREE_MAX_RUNS_PER_DAY}/day). Try again tomorrow, or connect your own device.`,
      };
    }
  }
