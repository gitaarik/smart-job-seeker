...
describe("canonFilters / filtersEqual", () => {
  ...
  it("normalizes a scalar value and a single-element array alike", () => {
    expect(filtersEqual({ sort_by: "newest" }, { sort_by: ["newest"] })).toBe(
      true,
    );
  });
  ...
  it("distinguishes genuinely different selections", () => {
    expect(
      filtersEqual({ experience_level: ["senior"] }, {
...
describe("computeInputHash", () => {
  it("is deterministic for identical inputs", () => {
    expect(computeInputHash(baseHashInput)).toBe(
      computeInputHash(baseHashInput),
    );
  });
  ...
  it("changes when a preference changes", () => {
    ...
    expect(computeInputHash(seniorityChanged)).not.toBe(
      computeInputHash(baseHashInput),
    );
    expect(computeInputHash(titleChanged)).not.toBe(
      computeInputHash(baseHashInput),
    );
    expect(computeInputHash(remoteToggled)).not.toBe(
      computeInputHash(baseHashInput),
    );
  });
