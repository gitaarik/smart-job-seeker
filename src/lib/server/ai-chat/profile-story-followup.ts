...
export async function createProfileStoryFollowup(
	...
	if (mode === 'review' || updateContent) {
		...
		if (story) {
			...
			historyMessages = await buildConversationMessages(STORY_VERSIONS, storyId, {
				noun: 'story',
				currentContent: currentStar
			});

			extraVariables = {
				storyContext,
				currentStar: currentStar || "(The applicant hasn't written anything yet.)"
			};
			promptType = mode === 'review' ? 'review_star_story' : 'followup_star_story';
