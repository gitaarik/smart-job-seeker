...
import { CHEATSHEET_VERSIONS, ensureBaselineVersion, recordVersion } from './entity-versions';
...
function parseSheetResponse(response: string | null): {
	text: string | null;
...
export async function createProfileCheatSheetFollowup(
	...
	if (mode === 'review' || updateContent) {
		...
		if (sheet) {
			...
			await ensureBaselineVersion(CHEATSHEET_VERSIONS, cheatSheetId, currentContent);
			...
			historyMessages = await buildConversationMessages(CHEATSHEET_VERSIONS, cheatSheetId, {
				noun: 'sheet',
				currentContent
			});
			...
			extraVariables = {
				sheetContext,
				currentSheet: currentContent || "(The applicant hasn't written anything yet.)"
			};
			promptType = mode === 'review' ? 'review_prep_sheet' : 'followup_prep_sheet';
			...
			if (mode !== 'review') {
				context = {
					query: {
						text: [followupRequest, sheetContext, currentContent].filter(Boolean).join('\n')
					},
				...
				if (aiChatResponse) {
					try {
						...
						if (typeof parsed.feedback === 'string') {
							...
							revised =
								typeof parsed.revisedText === 'string' ? parsed.revisedText.trim() || null : null;
						}
					} catch {
