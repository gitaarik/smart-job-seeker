 ...
 */
const PLACEHOLDER_TITLES = new Set(['new cheat sheet', 'untitled', 'untitled cheat sheet']);
...
export async function generateProfileCheatSheet(
	...
	if (mode === 'review') {
		variables.currentSheet = currentContent || "(The applicant hasn't written anything yet.)";
	} else {
	...
	if (mode === 'generate' || mode === 'auto') {
		...
		context = {
			query: {
				text: [topic, instructions ?? '', currentContent ?? ''].filter(Boolean).join('\n')
			},
	...
	try {
		...
		const commitContent = (mode === 'generate' || mode === 'auto') && sheetContent;
		await db
			.update(cheat_sheets)
			.set({
				...
				...(commitContent
					? {
							...
							...(suggestedTitle && isUnnamed(sheet.title) ? { title: suggestedTitle } : {})
						}
					: {})
			})
			.where(eq(cheat_sheets.id, cheatSheetId));
