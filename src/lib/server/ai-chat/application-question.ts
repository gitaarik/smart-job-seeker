...
export async function generateApplicationQuestionAnswer(
	...
	if (mode === 'generate' || mode === 'auto') {
		...
		if (aiChat.response) {
			try {
				const parsed = JSON.parse(aiChat.response);
				aiFeedback = typeof parsed.feedback === 'string' ? parsed.feedback : aiChat.response;
				answerText = typeof parsed.revisedText === 'string' ? parsed.revisedText : null;
			} catch {
				aiFeedback = aiChat.response;
			}
		}
	...
	try {
		...
		await db
			...
			.set({
				...
				...((mode === 'generate' || (mode === 'auto' && answerText)) && commitAnswer
					? { answer: answerText }
					: {})
			})
			.where(eq(application_questions.id, questionId));
