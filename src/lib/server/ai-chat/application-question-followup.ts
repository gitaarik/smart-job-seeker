 ...
 */
function parseAnswerResponse(response: string | null): {
	text: string | null;
...
export async function createApplicationQuestionFollowup(
	...
	if (mode === 'review' || updateContent) {
		...
		if (questionRecord) {
			...
			await ensureBaselineVersion(QUESTION_VERSIONS, questionId, questionRecord.answer);
			...
			if (applicationId != null) {
				...
				if (mode !== 'review') {
					context.query = {
						text: [followupRequest, questionRecord.question].filter(Boolean).join('\n')
					};
			...
			historyMessages = await buildConversationMessages(QUESTION_VERSIONS, questionId, {
				noun: 'answer',
				currentContent: currentAnswer
			});
	...
	return createEntityFollowup({
		...
		fetchEntity: (id) =>
			db.query.application_questions
				.findFirst({
				...
				if (aiChatResponse) {
					try {
						...
						if (parsed && typeof parsed.feedback === 'string') {
							...
							revisedText = typeof parsed.revisedText === 'string' ? parsed.revisedText : null;
						}
					} catch {
