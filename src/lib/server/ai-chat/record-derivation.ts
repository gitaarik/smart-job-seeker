...
export function coerceDerived(candidate: Candidate): {
  ...
  const record_type = rawType && recordTypeValues.includes(rawType)
    ? rawType
    : null;
  ...
  const event_date = rawDate && /^\d{4}-\d{2}-\d{2}$/.test(rawDate) &&
  ...
  if (Array.isArray(candidate.contacts)) {
    for (const entry of candidate.contacts) {
      ...
      contacts.push({
        name,
        role: rawRole && contactRoleValues.includes(rawRole)
          ? rawRole as RecordContact["role"]
          : null,
      });
    }
  }
...
export async function deriveRecordMetadata(
  ...
  try {
    ...
    const derived = coerceDerived(
      JSON.parse(result.aiChat.response) as Candidate,
    );
    ...
    await db.update(application_records)
