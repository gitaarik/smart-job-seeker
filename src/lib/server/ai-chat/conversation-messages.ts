...
function approximateTokens(messages: BaseMessage[]): number {
	return messages.reduce((total, m) => total + Math.ceil(String(m.content).length / 4) + 4, 0);
}

/** Replay one version trail as LangChain messages. */
function turnsToBaseMessages(turns: VersionTurn[], opts: HistoryOptions): BaseMessage[] {
	...
	for (const turn of turns) {
		...
		if (turn.source === 'manual_edit') {
			if (turn.content) {
				messages.push(
					new HumanMessage(`I wrote this version of the ${noun} myself:\n\n${draft(turn.content)}`)
				);
			}
			continue;
		}
