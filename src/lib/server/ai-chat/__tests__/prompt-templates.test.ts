...
describe('${applicationActivity} template \u2194 caller wiring', () => {
	...
	it('stays out of extraction prompts, which run on tight token budgets', () => {
		...
		for (const key of extractionPrompts) {
			...
			expect(referencing, `${key} must not carry the application history`).not.toContain(key);
		}
	});
	...
	it('guards writing prompts against inventing a shared history', () => {
		...
		const writingPrompts = referencing.filter((key) => !key.includes('cheat_sheet'));
		...
		for (const key of writingPrompts) {
			...
			expect(full, `${key} lacks the fabrication guard`).toMatch(
				/Never imply a conversation, meeting or relationship/
			);
		}
	});
	...
	it('tells interview-prep prompts to build on the earlier rounds', () => {
		for (const key of referencing.filter((k) => k.includes('cheat_sheet'))) {
			const t = promptTemplates[key];
			const full = `${t.system_prompt}\n${t.user_prompt}`;
			expect(full, `${key} lacks the build-on-earlier-rounds instruction`).toMatch(
				/records of earlier rounds/
			);
		}
	});
});
...
describe('${additionalContext} template ↔ caller wiring', () => {
	...
	it('is referenced by every template a caller supplies it to', () => {
		expect(referencing.sort()).toEqual(Object.keys(ADDITIONAL_CONTEXT_SUPPLIED_BY).sort());
	});

	it('is supplied by a caller for every template that references it', () => {
		const unsupplied = referencing.filter((key) => !(key in ADDITIONAL_CONTEXT_SUPPLIED_BY));
		expect(unsupplied).toEqual([]);
	});
...
describe('${relevantProjects} template ↔ caller wiring', () => {
	...
	it('is referenced by every template a caller supplies it to', () => {
		expect(referencing.sort()).toEqual(Object.keys(RELEVANT_PROJECTS_SUPPLIED_BY).sort());
	});

	it('is supplied by a caller for every template that references it', () => {
		const unsupplied = referencing.filter((key) => !(key in RELEVANT_PROJECTS_SUPPLIED_BY));
		expect(unsupplied).toEqual([]);
	});
...
describe('${relevantStories} template ↔ caller wiring', () => {
	...
	it('is referenced by every template a caller supplies it to', () => {
		expect(referencing.sort()).toEqual(Object.keys(RELEVANT_STORIES_SUPPLIED_BY).sort());
	});

	it('is supplied by a caller for every template that references it', () => {
		const unsupplied = referencing.filter((key) => !(key in RELEVANT_STORIES_SUPPLIED_BY));
		expect(unsupplied).toEqual([]);
	});
});
...
describe('${relevantApplicationTexts} template ↔ caller wiring', () => {
	...
	it('is referenced by every template a caller supplies it to', () => {
		expect(referencing.sort()).toEqual(Object.keys(RELEVANT_APPLICATION_TEXTS_SUPPLIED_BY).sort());
	});
