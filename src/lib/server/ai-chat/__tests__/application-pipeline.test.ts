...
describe('formatPipelineContext', () => {
	...
	it('renders every application given, not a selection', () => {
		const rows = Array.from({ length: 12 }, (_, i) => row({ id: i, company: `Co${i}` }));
	...
	it('shows the stated salary alone when it cannot be converted', () => {
		const out = formatPipelineContext([row({ salary: 'SGD 12,000/month', salaryAnnual: null })]);
		expect(out).toContain('SGD 12,000/month');
		expect(out).not.toContain('(~');
	});

	it('says so plainly when a job states no salary', () => {
		const out = formatPipelineContext([row({ salary: null, salaryAnnual: null })]);
		expect(out).toContain('no salary stated');
	});

	it('reports an unscored application as unscored rather than as zero', () => {
		const out = formatPipelineContext([row({ matchScore: null, matchRecommendation: null })]);
		expect(out).toContain('not scored');
		expect(out).not.toMatch(/match 0\b/);
	});

	it('carries how long the stage has been stuck', () => {
		expect(formatPipelineContext([row({ daysInStage: 19 })])).toContain('19d in stage');
		expect(formatPipelineContext([row({ daysInStage: null })])).toContain('age unknown');
	});
	...
	describe('without a current application (list pages)', () => {
		...
		it('gives both modes the same summary-not-the-whole-story guard', () => {
			...
			expect(formatPipelineContext([row({ isCurrent: true })])).toContain(guard);
		});

		it('keeps the guidance that does not depend on where the user is', () => {
			const out = formatPipelineContext([row()]);
			expect(out).toContain('RESPOND BY date is a deadline');
			expect(out).toContain('never present a conversion as what the employer offered');
		});

		it('switches back the moment a row is current', () => {
			const out = formatPipelineContext([row({ isCurrent: true }), row({ id: 2 })]);
	...
	it('survives an application with no job attached', () => {
		const out = formatPipelineContext([row({ id: 42, title: null, company: null })]);
	...
	it('flattens a multi-line summary so one application stays one block', () => {
		const out = formatPipelineContext([row({ summary: 'First line.\n\nSecond line.' })]);
...
describe('fitPipelineToBudget', () => {
	...
	const chars = (rows: PipelineRow[], o = {}) => formatPipelineContext(rows, o).length;
	...
	it('actually gets under the budget', () => {
		...
		expect(chars(out.rows, { omitted: out.omitted, shed: out.shed })).toBeLessThanOrEqual(budget);
	});
	...
	it("sheds the current application's details first", () => {
		...
		expect(out.rows.filter((r) => !r.isCurrent).some((r) => r.details.length)).toBe(true);
	});
	...
	it('does not report shed details as a missing history', () => {
		const rows = pipeline(20, { details: DETAILS });
		const out = fitPipelineToBudget(rows, chars(rows.map((r) => ({ ...r, details: [] }))));
		expect(out.shed).toBe(0);
		expect(formatPipelineContext(out.rows, { shed: out.shed })).not.toContain('without a summary');
	});
	...
	it('keeps the current row even when it sorted to the stale end', () => {
		...
		expect(formatPipelineContext(out.rows, { omitted: out.omitted })).toContain('THIS ONE');
	});
