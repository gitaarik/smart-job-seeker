...
export async function generateApplicationLetter(
	...
	if (!letter.application) {
		return {
			success: false,
			message: `Application letter ${letterId} does not have a linked application`
		};
	}
	...
	if (!promptMap) {
		return {
			success: false,
			message: `Unknown letter type: ${letterType}. Cannot determine which AI prompt to use.`
		};
	}
	...
	return {
		success: true,
		message: `${letterType.replace('_', ' ')} generated for application letter ID ${letterId}`
	};
}
