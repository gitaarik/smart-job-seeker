...
function setup(
  ...
  (db.query.tech_skill_categories.findMany as any).mockResolvedValue(
    categories,
  );
}
...
describe("getHiddenRequiredSkills", () => {
  ...
  it("does not flag a version the skill was re-admitted on", async () => {
    ...
    expect(names(hidden[hiddenSkillsKey("resume", "")])).toEqual([
      "Kubernetes",
    ]);
  });
  ...
  it("respects a base-template restriction, and won't promise a fix", async () => {
    ...
    expect(hidden[hiddenSkillsKey("resume", "backend")][0].liftable).toBe(
      false,
    );
  });
