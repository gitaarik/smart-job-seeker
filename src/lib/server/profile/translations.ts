...
export async function loadTranslator(
  ...
  return {
    locale,
    isBase: false,
    t: (entity, id, field, base) =>
      map.get(translationKey(entity, id, field)) ?? base,
  };
}
 ...
 */
export async function loadOwnedEntityIds(
  profileId: number,
): Promise<Record<string, Set<number>>> {
  const [we, wea, cats, edu, sp, spa] = await Promise.all([
    db.select({ id: work_experiences.id }).from(work_experiences)
    ...
    db.select({ id: side_projects.id }).from(side_projects)
