 ...
 */
export function stripHtmlForLlm(html: string, options: StripHtmlOptions = {}): string {
			...
			Object.keys(attrs).forEach((attr) => {
				if (attr === 'class') {
					...
					const filteredClasses = classes.filter((cls) =>
						classWhitelist.some((pattern) => cls.toLowerCase().includes(pattern))
					);
		...
		$('div, span').each((_, elem) => {
			...
			if (!hasAttrs) {
				...
				if (children.length === 1) {
					...
					const childHasAttrs = childAttrs && Object.keys(childAttrs).length > 0;

					// Only flatten if child also has no attributes (both are purely structural)
					if (
						!childHasAttrs &&
						(child.prop('tagName') === 'DIV' || child.prop('tagName') === 'SPAN')
					) {
	...
	while (changed && iterations < maxIterations) {
		...
		$('*').each((_, elem) => {
			...
			const attrs = element.attr();
			const hasPreservedAttrs =
				attrs && Object.keys(attrs).some((attr) => keepAttributes.has(attr) || attr === 'class');
	...
	if (cleaned.length > maxChars) {
		console.warn(`⚠️  HTML too large (${cleaned.length} chars), truncating to ${maxChars} chars`);
