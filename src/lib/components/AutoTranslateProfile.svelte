...
<div class="rounded-lg border border-[var(--dash-border)] bg-[var(--dash-card)] p-4">
	<h3 class="flex items-center gap-1.5 text-sm font-semibold text-[var(--dash-text)]">
		✨ AI translation
	</h3>
	<p class="mt-1 mb-3 text-xs text-[var(--dash-text-muted)]">
		Auto-translate every text field into another language. Fills the inline language tabs across
		your profile; existing translations are kept unless you overwrite them.
	</p>
	<div class="flex flex-wrap items-center gap-2">
		<select
			bind:value={locale}
			disabled={busy}
			class="rounded-md border border-[var(--dash-border)] bg-[var(--dash-bg)] px-2 py-1.5 text-sm text-[var(--dash-text)] focus:ring-2 focus:ring-[var(--dash-primary)] focus:outline-none"
		>
		...
		<button
			...
			onclick={run}
			disabled={busy}
			class="rounded-md bg-[var(--dash-primary)] px-3 py-1.5 text-sm font-medium text-white hover:bg-[var(--dash-primary-hover)] disabled:cursor-not-allowed disabled:opacity-50"
		>
