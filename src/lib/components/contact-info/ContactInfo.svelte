<script lang="ts">
	...
	import { faCircleNotch, faEnvelope, faPhone } from '@fortawesome/free-solid-svg-icons';

	import { faSignalMessenger, faTelegram, faWhatsapp } from '@fortawesome/free-brands-svg-icons';
	...
	onMount(() => {
		if (browser) {
			setInterval(() => {
				if (turnstileContainer && turnstileContainer.getBoundingClientRect().height > 0) {
					isLoading = false;
				}
			}, 100);
...
<div class="flex h-full flex-col items-center">
	{#if !isHumanState.value}
		{#if isLoadError || isVerifyError}
			<div class="text-crimson px-4 text-center md:px-8">
				<p>
	...
	{:else}
		<div class="max-xs:flex-col xs:gap-4 flex w-full place-content-evenly gap-6 px-4 text-center">
			{#if profile.phone_number}
				<div>
					...
					<a href="tel:{profile.phone_number}" class="hover:text-teal underline"
						...
						<p class="mt-2 flex justify-center gap-4 text-xl">
							...
							{#if profile.whatsapp_number}
								<a
									...
									target="_blank"
									title="WhatsApp"
									class="hover:text-teal"><FontAwesomeIcon icon={faWhatsapp} /></a
								>
							{/if}
							...
							{#if profile.telegram_username}
								<a
									...
									target="_blank"
									title="Telegram"
									class="hover:text-teal"><FontAwesomeIcon icon={faTelegram} /></a
								>
							{/if}
						</p>
					{/if}
			...
			{#if profile.email_address}
				<div>
					...
					<a href="mailto:{profile.email_address}" class="hover:text-teal underline"
						>{profile.email_address}</a
					>

					<p class="xs:mt-4 mt-2 text-sm/6">
