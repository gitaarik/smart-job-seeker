...
describe('formatOfferAmount', () => {
	...
	it('formats a stated amount in its stated currency', () => {
		const text = formatOfferAmount(offer({ base: 92000, currency: 'EUR', period: 'year' }));
	...
	it('shows an unrecognised currency verbatim instead of throwing', () => {
		...
		expect(() => formatOfferAmount(offer({ base: 92000, currency: 'DOLLARS' }))).not.toThrow();
		expect(formatOfferAmount(offer({ base: 92000, currency: 'DOLLARS' }))).toBe('92,000 DOLLARS');
	});

	it('omits the period when none was stated', () => {
		expect(formatOfferAmount(offer({ base: 5000, currency: 'EUR' }))).not.toContain('/');
	});
});
...
describe('hasOfferContent', () => {
	it('rejects null and an offer carrying only a currency', () => {
		...
		expect(hasOfferContent(offer({ currency: 'EUR', period: 'year' }))).toBe(false);
	});
