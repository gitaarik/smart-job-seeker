 ...
 */
export function resolveTimeFormat(raw: string | null, timezone: string | null): TimeFormat {
 ...
 * "Jan 5, 2025"
 */
export function formatDate(date: Date | string | null, opts?: FormatOpts): string {
	const d = toDate(date);
 ...
 */
export function formatDateLong(date: Date | string | null, opts?: FormatOpts): string {
	const d = toDate(date);
	if (!d) return opts?.fallback ?? 'N/A';
	return d.toLocaleDateString('en-US', {
		weekday: 'long',
 ...
 */
export function formatMonthDay(date: Date | string | null, opts?: FormatOpts): string {
	const d = toDate(date);
	if (!d) return opts?.fallback ?? 'N/A';
	return d.toLocaleDateString('en-US', {
		month: 'short',
 ...
 * "Mon, Jan 5"
 */
export function formatDateShort(date: Date | string | null, opts?: FormatOpts): string {
	const d = toDate(date);
	if (!d) return opts?.fallback ?? 'N/A';
	return d.toLocaleDateString('en-US', {
		weekday: 'short',
 ...
 */
export function buildHourOptions(timeFormat: TimeFormat): { value: number; label: string }[] {
