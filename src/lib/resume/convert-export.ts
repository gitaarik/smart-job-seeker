...
function normalizeProficiency(value?: string): Proficiency | undefined {
	if (!value) return undefined;
	return validProficiencies.includes(value as Proficiency) ? (value as Proficiency) : undefined;
}
...
export function convertExportToResumeData(exported: ExportedProfile): ResumeData {
	...
	return {
		...
		work: p.work_experiences?.map((w) => ({
			...
			summary: w.summary,
			achievements: w.achievements?.filter((a) => a.description).map((a) => a.description!) || [],
			technologies: w.technologies?.filter((t) => t.name).map((t) => t.name!) || []
		})),
		...
		skills: p.tech_skill_categories?.map((cat) => ({
			...
			skills:
				cat.tech_skills?.map((s) => ({
					...
					yearsExperience: s.years_experience ? parseFloat(s.years_experience) : undefined
				})) || []
		})),
		...
		projects: p.side_projects?.map((proj) => ({
			...
			technologies: proj.technologies?.filter((t) => t.name).map((t) => t.name!) || [],
