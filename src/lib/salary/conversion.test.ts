...
describe("compareSalary", () => {
  it("returns unknown when the job has no salary range", () => {
    expect(
      compareSalary(5000, "EUR", "month", null, null, "EUR", "month", RATES),
    ).toBe("unknown");
  });
  ...
  it("compares across currencies using the injected rates", () => {
    ...
    expect(
      compareSalary(5000, "EUR", "month", 5000, 7560, "USD", "month", RATES),
    ).toBe("within");
    // The same ask is clearly below a much higher USD range.
    expect(
      compareSalary(5000, "EUR", "month", 9000, 11000, "USD", "month", RATES),
    ).toBe("below");
  });
  ...
  it("degrades to unknown (never guesses) when the rate pair is missing", () => {
    ...
    expect(
      compareSalary(5000, "EUR", "month", 600000, 800000, "JPY", "month", RATES),
    ).toBe("unknown");
    expect(
      compareSalary(5000, "EUR", "month", 5400, 7560, "USD", "month", {}),
    ).toBe("unknown");
  });

  it("still works same-currency even with an empty rate map", () => {
    expect(
      compareSalary(5000, "EUR", "month", 4000, 6000, "EUR", "month", {}),
    ).toBe("within");
  });
});
