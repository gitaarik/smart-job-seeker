...
- Show which app connected with each API key

### Fixed

- Release process now accepts uncommitted changelog entries for the version
...
- Backfill job skills for postings where extraction returned nothing

### Fixed

- Extract job skills from prose postings in any field

## [0.26.1] - 2026-08-31

3 commits since v0.26.0.

### Fixed

- Scraper now recovers with Enter when a submit click leaves the query unapplied
...
- npm shortcuts for corpus proposers and checks

### Changed

- Template override field now collapses when not configured
...
- Demo profile cloning now copies job matches from the source for improved performance

### Fixed

- Silent failures in file reaper and render bypass
- Escape blur, dismissal loop, and stale handle issues in scraper

## [0.23.0] - 2026-08-23

### Added

- 2 commits since v0.22.0.
- MCP agents can now create applications and attach files
- Agents have direct access to the review queue via MCP

## [0.22.0] - 2026-08-23

9 commits since v0.21.0.

### Added

- Comprehensive user guide pages covering interview prep, salary negotiation, job matching, credits, data export, resume management, and connecting your own AI via MCP

### Fixed

- Search form no longer clears when Escape is pressed unintentionally
...
- Fixed registration gate incorrectly blocking in-process signups
- Fixed job region classification gaps
...
- Login blocks that aren't recognized by built-in rules are now classified using an LLM fallback for better accuracy
...
- Fixed bundling of ops scripts that use relative paths to reference app source

### Removed

- Removed dead test-html-utils script
...
- Scraper now properly dismisses blocking overlays on the keywords-URL path

## [0.10.3] - 2026-08-09

7 commits since v0.10.2.

### Added

- Search-URL templates for Upwork and keywords across job platforms

### Fixed

- Scraper no longer reports lost searches as successful runs
- Scraper handles field value swaps and focus-based form submission
- Scraper re-resolves targets when popup dismissal triggers re-renders
...
- Profile deletion now cascades properly and matcher skips deleted owner profiles to prevent orphaned data
