...
const BUDGET = process.env.THINKING_BUDGET ? Number(process.env.THINKING_BUDGET) : undefined;
...
const schema = buildProposalSchema(['edit_job_details', 'edit_job_description', 'edit_job_skills']);
...
for (let n = 0; n < RUNS; n++) {
	...
	try {
		...
		const thinking = usage ? usage.total_tokens - usage.input_tokens - usage.output_tokens : 0;
