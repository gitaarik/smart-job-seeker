 ...
 */
const ALL_SEED_TABLES = [...SEED_TABLES.reference, ...SEED_TABLES.auth, ...SEED_TABLES.profile];
...
function buildScopedCopy() {
	...
	execFileSync('npx', ['tsx', 'scripts/migrate-deploy.ts'], {
		env: {
			...pgEnv,
			SJS_DATABASE_URL: `postgres://${DB_USER}:${DB_PASSWORD}@${DB_HOST}:${DB_PORT}/${BUILD_DB}`
		},
	...
	for (const table of ALL_SEED_TABLES) {
       ...
       WHERE table_schema = 'public' AND table_name = '${table}'`
		...
		const rows = psqlOn(DB_NAME, ['-q', '-c', `\\copy ${table} (${columns}) TO STDOUT`]);
...
`;
	...
	sql += execFileSync(
		...
		{ env: pgEnv, maxBuffer: 200 * 1024 * 1024 }
	).toString();
