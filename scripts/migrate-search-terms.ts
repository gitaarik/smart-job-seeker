...
function extractFromTemplate(urlTemplate: string, actualUrl: string): string | null {
	let template: URL;
	let actual: URL;
	try {
...
async function main() {
	...
	const presetIds = Array.from(
		new Set(candidates.map((t) => t.preset_id).filter((x): x is number => x !== null))
	);
	const presets =
		presetIds.length > 0
	...
	for (const task of candidates) {
		...
		if (!extracted) {
			skipped++;
			console.warn(
				`  task ${task.id}: could not extract — search_url=${task.search_url.slice(0, 80)}`
			);
			continue;
		}
