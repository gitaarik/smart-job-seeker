...
const getWeather = tool(async ({ city }: { city: string }) => `18C and raining in ${city}`, {
	name: 'get_weather',
...
try {
	const res = await model
		...
		.invoke([new HumanMessage('What is the weather in Amsterdam? Use the tool.')]);
...
const proposeEdit = tool(async () => 'stored', {
	name: 'propose_edit',
...
try {
	...
	report(
		...
		calls.length === 1 && Array.isArray(args?.changes) && args!.changes!.length > 0,
