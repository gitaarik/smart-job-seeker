...
async function main() {
  ...
  const noLocation = rest.filter(
    (r) => !r.office_location || r.office_location.trim() === "",
  );
  const withLocation = rest.filter(
    (r) => r.office_location && r.office_location.trim() !== "",
  );
  ...
  const pendingBackfill = stillAPlace.filter(
    (r) => classifyRegion(r.office_location) !== null,
  );
  const realMisses = stillAPlace.filter(
    (r) => classifyRegion(r.office_location) === null,
  );

  console.log(`jobs: ${total}\n`);
  console.log(`  region set          ${String(classified.length).padStart(6)}  ${pct(classified.length, total)}`);
  console.log(`  region null         ${String(unclassified.length).padStart(6)}  ${pct(unclassified.length, total)}\n`);
  console.log("of the nulls:\n");
  console.log(`  remote job                  ${String(remote.length).padStart(6)}   correct — names no place`);
  console.log(`  no office_location          ${String(noLocation.length).padStart(6)}   correct — nothing to derive from`);
  console.log(`  location is an arrangement  ${String(arrangementNotPlace.length).padStart(6)}   run backfill-work-location`);
  console.log(`  classifier reads it now     ${String(pendingBackfill.length).padStart(6)}   run backfill-job-regions`);
  console.log(`  ── unclassified location    ${String(realMisses.length).padStart(6)}   ← the only defect count`);
