...
async function main() {
	...
	if (credentialId === null) {
		...
		if (!firstCred) {
			console.error(`No credentials for platform ${platformId} — create one first`);
			process.exit(1);
		}
	...
	if (!cred || cred.platform_id !== platform.id) {
		console.error(`Credential ${credentialId} not found or belongs to a different platform`);
		process.exit(1);
	}
