...
const PROBES = ['React', 'k8s', 'JS', 'Python', 'PostgreSQL', 'communication'];
...
async function main() {
	...
	console.log(`current SJS_EMBEDDING_SKILL_THRESHOLD default: ${config.embeddingSkillThreshold}\n`);
	...
	if (doBackfill) {
		...
		for (let i = 0; i < skills.length; i += CHUNK) {
			...
			if ((i / CHUNK) % 10 === 0) {
				console.log(
					`  ${Math.min(i + CHUNK, skills.length)}/${skills.length} (embedded: ${added})`
				);
			}
	...
	console.log(
		`cosine ops per expansion (30-skill profile): ${((30 * vocab.length * dims) / 1e6).toFixed(
			0
		)}M multiply-adds\n`
	);
	...
	console.log(`=== expansion factor per probe skill (how many vocab terms it pulls in) ===`);
	const header = ['skill'.padEnd(16), ...THRESHOLDS.map((t) => t.toFixed(2).padStart(6))].join('');
	console.log(header);
	console.log('-'.repeat(header.length));

	for (const probe of PROBES) {
		const match = vocab.find((v) => v.label.toLowerCase() === probe.toLowerCase());
		if (!match) {
		...
		console.log([probe.padEnd(16), ...counts.map((c) => String(c).padStart(6))].join(''));
	}
	...
	console.log(
		`n=${sample.length}  p50=${pct(0.5)}  p90=${pct(0.9)}  p99=${pct(
			0.99
	...
	if (doExportFixture) {
		...
		const stride = Math.max(1, Math.floor(vocab.length / FIXTURE_RANDOM_SAMPLE));
		for (
			let i = 0;
			i < vocab.length && picked.size < FIXTURE_ANCHORS.length + FIXTURE_RANDOM_SAMPLE;
			i += stride
		) {
		...
		const sims = entries.map((a) =>
			entries.map((b) => Number(cosineSimilarity(a.vector, b.vector).toFixed(4)))
		);
		...
		const filePath = fileURLToPath(
			new URL('../src/lib/server/job/__tests__/fixtures/skill-similarities.json', import.meta.url)
		);
		...
		if (missingAnchors.length > 0) {
			console.log(
				`!! anchors missing from vocabulary (not in job data): ${missingAnchors.join(', ')}`
			);
		}
	}
