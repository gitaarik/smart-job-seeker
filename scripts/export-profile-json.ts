...
async function exportProfile(profileId: string, exportToFile: boolean = true): Promise<void> {
	try {
		console.log(`Exporting profile: ${profileId}`);

		const id = parseInt(profileId, 10);
		const { data: exportData, profileName } = await buildProfileJsonExport(id);
		...
		console.log(`   - Profile versions: ${exportData.profile.profile_versions.length}`);
		console.log(`   - Highlights: ${exportData.profile.highlights.length}`);
		console.log(`   - Tech skill categories: ${exportData.profile.tech_skill_categories.length}`);
		console.log(`   - Work experiences: ${exportData.profile.work_experiences.length}`);
		console.log(`   - Side projects: ${exportData.profile.side_projects.length}`);
		...
		console.log(`   - Project stories: ${exportData.profile.project_stories.length}`);
...
async function main() {
	...
	if (!providedProfileId) {
		...
		if (!defaultId) {
			console.error('❌ Error: No profile ID provided and no default profile is set');
